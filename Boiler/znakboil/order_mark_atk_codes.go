...
var OrderMarkAtkCodeWhere = struct {
	...
	ID:                     whereHelperint64{field: "\"order_mark_atk_codes\".\"id\""},
	IDOrderMarkAtk:         whereHelpernull_Int64{field: "\"order_mark_atk_codes\".\"id_order_mark_atk\""},
	IDOrderMarkUtilisation: whereHelpernull_Int64{field: "\"order_mark_atk_codes\".\"id_order_mark_utilisation\""},
	IDOrderMarkCodes:       whereHelpernull_Int64{field: "\"order_mark_atk_codes\".\"id_order_mark_codes\""},
	SerialNumber:           whereHelpernull_String{field: "\"order_mark_atk_codes\".\"serial_number\""},
	Code:                   whereHelpernull_String{field: "\"order_mark_atk_codes\".\"code\""},
	Status:                 whereHelpernull_String{field: "\"order_mark_atk_codes\".\"status\""},
	Gtin:                   whereHelpernull_String{field: "\"order_mark_atk_codes\".\"gtin\""},
	ReportDate:             whereHelpernull_String{field: "\"order_mark_atk_codes\".\"report_date\""},
	ReportID:               whereHelpernull_String{field: "\"order_mark_atk_codes\".\"report_id\""},
}
...
var (
	...
	orderMarkAtkCodeColumnsWithoutDefault = []string{}
	orderMarkAtkCodeColumnsWithDefault    = []string{"id", "id_order_mark_atk", "id_order_mark_utilisation", "id_order_mark_codes", "serial_number", "code", "status", "gtin", "report_date", "report_id"}
...
func OrderMarkAtkCodes(mods ...qm.QueryMod) orderMarkAtkCodeQuery {
	mods = append(mods, qm.From("\"order_mark_atk_codes\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"order_mark_atk_codes\".*"})
	}
...
func FindOrderMarkAtkCodeG(ctx context.Context, iD int64, selectCols ...string) (*OrderMarkAtkCode, error) {
...
func FindOrderMarkAtkCode(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*OrderMarkAtkCode, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"order_mark_atk_codes\" where \"id\"=?", sel,
	)
...
func (o *OrderMarkAtkCode) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"order_mark_atk_codes\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"order_mark_atk_codes\" %sDEFAULT VALUES%s"
		}
...
func (o *OrderMarkAtkCode) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"order_mark_atk_codes\" SET %s WHERE %s",
			strmangle.SetParamNames("\"", "\"", 0, wl),
			strmangle.WhereClause("\"", "\"", 0, orderMarkAtkCodePrimaryKeyColumns),
		)
...
func (o OrderMarkAtkCodeSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"order_mark_atk_codes\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 0, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, orderMarkAtkCodePrimaryKeyColumns, len(o)))
...
func (o *OrderMarkAtkCode) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *OrderMarkAtkCode) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 {
			conflict = make([]string, len(orderMarkAtkCodePrimaryKeyColumns))
			copy(conflict, orderMarkAtkCodePrimaryKeyColumns)
		}
		cache.query = buildUpsertQuerySQLite(dialect, "\"order_mark_atk_codes\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(orderMarkAtkCodeType, orderMarkAtkCodeMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *OrderMarkAtkCode) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"order_mark_atk_codes\" WHERE \"id\"=?"
...
func (o OrderMarkAtkCodeSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"order_mark_atk_codes\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, orderMarkAtkCodePrimaryKeyColumns, len(o))
...
func (o *OrderMarkAtkCodeSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"order_mark_atk_codes\".* FROM \"order_mark_atk_codes\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, orderMarkAtkCodePrimaryKeyColumns, len(*o))
...
func OrderMarkAtkCodeExistsG(ctx context.Context, iD int64) (bool, error) {
	return OrderMarkAtkCodeExists(ctx, boil.GetContextDB(), iD)
}

// OrderMarkAtkCodeExists checks if the OrderMarkAtkCode row exists.
func OrderMarkAtkCodeExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"order_mark_atk_codes\" where \"id\"=? limit 1)"
