...
var ProductCardsIdentifierWhere = struct {
	...
	ID:             whereHelperint64{field: "\"product_cards_identifiers\".\"id\""},
	IDProductCards: whereHelpernull_Int64{field: "\"product_cards_identifiers\".\"id_product_cards\""},
	Value:          whereHelpernull_String{field: "\"product_cards_identifiers\".\"value\""},
	Type:           whereHelpernull_String{field: "\"product_cards_identifiers\".\"type\""},
	Multiplier:     whereHelpernull_Int64{field: "\"product_cards_identifiers\".\"multiplier\""},
	Level:          whereHelpernull_String{field: "\"product_cards_identifiers\".\"level\""},
}
...
var (
	...
	productCardsIdentifierColumnsWithoutDefault = []string{}
	productCardsIdentifierColumnsWithDefault    = []string{"id", "id_product_cards", "value", "type", "multiplier", "level"}
...
func ProductCardsIdentifiers(mods ...qm.QueryMod) productCardsIdentifierQuery {
	mods = append(mods, qm.From("\"product_cards_identifiers\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"product_cards_identifiers\".*"})
	}
...
func FindProductCardsIdentifierG(ctx context.Context, iD int64, selectCols ...string) (*ProductCardsIdentifier, error) {
...
func FindProductCardsIdentifier(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*ProductCardsIdentifier, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"product_cards_identifiers\" where \"id\"=?", sel,
	)
...
func (o *ProductCardsIdentifier) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"product_cards_identifiers\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"product_cards_identifiers\" %sDEFAULT VALUES%s"
		}
...
func (o *ProductCardsIdentifier) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"product_cards_identifiers\" SET %s WHERE %s",
			strmangle.SetParamNames("\"", "\"", 0, wl),
			strmangle.WhereClause("\"", "\"", 0, productCardsIdentifierPrimaryKeyColumns),
		)
...
func (o ProductCardsIdentifierSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"product_cards_identifiers\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 0, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, productCardsIdentifierPrimaryKeyColumns, len(o)))
...
func (o *ProductCardsIdentifier) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *ProductCardsIdentifier) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 {
			conflict = make([]string, len(productCardsIdentifierPrimaryKeyColumns))
			copy(conflict, productCardsIdentifierPrimaryKeyColumns)
		}
		cache.query = buildUpsertQuerySQLite(dialect, "\"product_cards_identifiers\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(productCardsIdentifierType, productCardsIdentifierMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *ProductCardsIdentifier) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"product_cards_identifiers\" WHERE \"id\"=?"
...
func (o ProductCardsIdentifierSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"product_cards_identifiers\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, productCardsIdentifierPrimaryKeyColumns, len(o))
...
func (o *ProductCardsIdentifierSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"product_cards_identifiers\".* FROM \"product_cards_identifiers\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, productCardsIdentifierPrimaryKeyColumns, len(*o))
...
func ProductCardsIdentifierExistsG(ctx context.Context, iD int64) (bool, error) {
...
func ProductCardsIdentifierExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"product_cards_identifiers\" where \"id\"=? limit 1)"
