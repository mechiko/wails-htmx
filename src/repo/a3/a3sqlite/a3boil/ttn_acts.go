...
var TTNActWhere = struct {
	...
	ID:         whereHelperint64{field: "\"ttn_acts\".\"id\""},
	IDTTN:      whereHelpernull_Int64{field: "\"ttn_acts\".\"id_ttn\""},
	ActType:    whereHelpernull_String{field: "\"ttn_acts\".\"act_type\""},
	ActNumber:  whereHelpernull_String{field: "\"ttn_acts\".\"act_number\""},
	ActDate:    whereHelpernull_String{field: "\"ttn_acts\".\"act_date\""},
	ActRegID:   whereHelpernull_String{field: "\"ttn_acts\".\"act_reg_id\""},
	ActComment: whereHelpernull_String{field: "\"ttn_acts\".\"act_comment\""},
	Status:     whereHelpernull_String{field: "\"ttn_acts\".\"status\""},
	ReplyID:    whereHelpernull_String{field: "\"ttn_acts\".\"reply_id\""},
	XML:        whereHelpernull_String{field: "\"ttn_acts\".\"xml\""},
}
...
var (
	...
	ttnActColumnsWithoutDefault = []string{}
	ttnActColumnsWithDefault    = []string{"id", "id_ttn", "act_type", "act_number", "act_date", "act_reg_id", "act_comment", "status", "reply_id", "xml"}
...
func TTNActs(mods ...qm.QueryMod) ttnActQuery {
	mods = append(mods, qm.From("\"ttn_acts\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"ttn_acts\".*"})
	}
...
func FindTTNActG(ctx context.Context, iD int64, selectCols ...string) (*TTNAct, error) {
...
func FindTTNAct(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*TTNAct, error) {
	ttnActObj := &TTNAct{}
	...
	query := fmt.Sprintf(
		"select %s from \"ttn_acts\" where \"id\"=?", sel,
	)
...
func (o *TTNAct) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"ttn_acts\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"ttn_acts\" %sDEFAULT VALUES%s"
		}
...
func (o *TTNAct) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"ttn_acts\" SET %s WHERE %s",
			strmangle.SetParamNames("\"", "\"", 0, wl),
			strmangle.WhereClause("\"", "\"", 0, ttnActPrimaryKeyColumns),
		)
...
func (o TTNActSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"ttn_acts\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 0, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, ttnActPrimaryKeyColumns, len(o)))
...
func (o *TTNAct) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *TTNAct) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 {
			conflict = make([]string, len(ttnActPrimaryKeyColumns))
			copy(conflict, ttnActPrimaryKeyColumns)
		}
		cache.query = buildUpsertQuerySQLite(dialect, "\"ttn_acts\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(ttnActType, ttnActMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *TTNAct) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"ttn_acts\" WHERE \"id\"=?"
...
func (o TTNActSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"ttn_acts\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, ttnActPrimaryKeyColumns, len(o))
...
func (o *TTNActSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"ttn_acts\".* FROM \"ttn_acts\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, ttnActPrimaryKeyColumns, len(*o))
...
func TTNActExistsG(ctx context.Context, iD int64) (bool, error) {
	return TTNActExists(ctx, boil.GetContextDB(), iD)
}

// TTNActExists checks if the TTNAct row exists.
func TTNActExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"ttn_acts\" where \"id\"=? limit 1)"
