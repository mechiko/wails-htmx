...
var ImportProductWhere = struct {
	...
	ID:                     whereHelperint64{field: "\"import_products\".\"id\""},
	IDImportReports:        whereHelpernull_Int64{field: "\"import_products\".\"id_import_reports\""},
	ProductFullName:        whereHelpernull_String{field: "\"import_products\".\"product_full_name\""},
	ProductCapacity:        whereHelpernull_String{field: "\"import_products\".\"product_capacity\""},
	ProductAlcVolume:       whereHelpernull_String{field: "\"import_products\".\"product_alc_volume\""},
	ProductAlcVolumeMin:    whereHelpernull_String{field: "\"import_products\".\"product_alc_volume_min\""},
	ProductAlcVolumeMax:    whereHelpernull_String{field: "\"import_products\".\"product_alc_volume_max\""},
	...
	ProductComment:         whereHelpernull_String{field: "\"import_products\".\"product_comment\""},
	ProducerType:           whereHelpernull_String{field: "\"import_products\".\"producer_type\""},
	ProducerClientRegID:    whereHelpernull_String{field: "\"import_products\".\"producer_client_reg_id\""},
	ProducerInn:            whereHelpernull_String{field: "\"import_products\".\"producer_inn\""},
	ProducerKPP:            whereHelpernull_String{field: "\"import_products\".\"producer_kpp\""},
	ProducerFullName:       whereHelpernull_String{field: "\"import_products\".\"producer_full_name\""},
	ProducerShortName:      whereHelpernull_String{field: "\"import_products\".\"producer_short_name\""},
	ProducerCountryCode:    whereHelpernull_String{field: "\"import_products\".\"producer_country_code\""},
	ProducerRegionCode:     whereHelpernull_String{field: "\"import_products\".\"producer_region_code\""},
	ProducerDescription:    whereHelpernull_String{field: "\"import_products\".\"producer_description\""},
}
...
func ImportProducts(mods ...qm.QueryMod) importProductQuery {
	mods = append(mods, qm.From("\"import_products\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"import_products\".*"})
	}
...
func FindImportProductG(ctx context.Context, iD int64, selectCols ...string) (*ImportProduct, error) {
...
func FindImportProduct(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*ImportProduct, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"import_products\" where \"id\"=?", sel,
	)
...
func (o *ImportProduct) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"import_products\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"import_products\" %sDEFAULT VALUES%s"
		}
...
func (o *ImportProduct) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"import_products\" SET %s WHERE %s",
			strmangle.SetParamNames("\"", "\"", 0, wl),
			strmangle.WhereClause("\"", "\"", 0, importProductPrimaryKeyColumns),
		)
...
func (o ImportProductSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"import_products\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 0, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, importProductPrimaryKeyColumns, len(o)))
...
func (o *ImportProduct) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *ImportProduct) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 {
			conflict = make([]string, len(importProductPrimaryKeyColumns))
			copy(conflict, importProductPrimaryKeyColumns)
		}
		cache.query = buildUpsertQuerySQLite(dialect, "\"import_products\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(importProductType, importProductMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *ImportProduct) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"import_products\" WHERE \"id\"=?"
...
func (o ImportProductSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"import_products\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, importProductPrimaryKeyColumns, len(o))
...
func (o *ImportProductSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"import_products\".* FROM \"import_products\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, importProductPrimaryKeyColumns, len(*o))
...
func ImportProductExistsG(ctx context.Context, iD int64) (bool, error) {
	return ImportProductExists(ctx, boil.GetContextDB(), iD)
}

// ImportProductExists checks if the ImportProduct row exists.
func ImportProductExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"import_products\" where \"id\"=? limit 1)"
