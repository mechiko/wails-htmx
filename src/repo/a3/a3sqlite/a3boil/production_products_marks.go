...
type ProductionProductsMark struct {
	...
	IDProductionProductsBoxes null.Int64  `boil:"id_production_products_boxes" json:"id_production_products_boxes,omitempty" toml:"id_production_products_boxes" yaml:"id_production_products_boxes,omitempty"`
...
var ProductionProductsMarkWhere = struct {
	...
	IDProductionProductsBoxes whereHelpernull_Int64
	Mark                      whereHelpernull_String
}{
	ID:                        whereHelperint64{field: "\"production_products_marks\".\"id\""},
	IDProductionProducts:      whereHelpernull_Int64{field: "\"production_products_marks\".\"id_production_products\""},
	IDProductionProductsBoxes: whereHelpernull_Int64{field: "\"production_products_marks\".\"id_production_products_boxes\""},
	Mark:                      whereHelpernull_String{field: "\"production_products_marks\".\"mark\""},
}
...
var (
	...
	productionProductsMarkColumnsWithoutDefault = []string{}
	productionProductsMarkColumnsWithDefault    = []string{"id", "id_production_products", "id_production_products_boxes", "mark"}
...
func ProductionProductsMarks(mods ...qm.QueryMod) productionProductsMarkQuery {
	mods = append(mods, qm.From("\"production_products_marks\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"production_products_marks\".*"})
	}
...
func FindProductionProductsMarkG(ctx context.Context, iD int64, selectCols ...string) (*ProductionProductsMark, error) {
...
func FindProductionProductsMark(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*ProductionProductsMark, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"production_products_marks\" where \"id\"=?", sel,
	)
...
func (o *ProductionProductsMark) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"production_products_marks\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"production_products_marks\" %sDEFAULT VALUES%s"
		}
...
func (o *ProductionProductsMark) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"production_products_marks\" SET %s WHERE %s",
			strmangle.SetParamNames("\"", "\"", 0, wl),
			strmangle.WhereClause("\"", "\"", 0, productionProductsMarkPrimaryKeyColumns),
		)
...
func (o ProductionProductsMarkSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"production_products_marks\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 0, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, productionProductsMarkPrimaryKeyColumns, len(o)))
...
func (o *ProductionProductsMark) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *ProductionProductsMark) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 {
			conflict = make([]string, len(productionProductsMarkPrimaryKeyColumns))
			copy(conflict, productionProductsMarkPrimaryKeyColumns)
		}
		cache.query = buildUpsertQuerySQLite(dialect, "\"production_products_marks\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(productionProductsMarkType, productionProductsMarkMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *ProductionProductsMark) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"production_products_marks\" WHERE \"id\"=?"
...
func (o ProductionProductsMarkSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"production_products_marks\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, productionProductsMarkPrimaryKeyColumns, len(o))
...
func (o *ProductionProductsMarkSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"production_products_marks\".* FROM \"production_products_marks\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, productionProductsMarkPrimaryKeyColumns, len(*o))
...
func ProductionProductsMarkExistsG(ctx context.Context, iD int64) (bool, error) {
...
func ProductionProductsMarkExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"production_products_marks\" where \"id\"=? limit 1)"
