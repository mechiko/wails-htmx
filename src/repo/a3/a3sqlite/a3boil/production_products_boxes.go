...
type ProductionProductsBox struct {
	...
	IDProductionProductsPallets null.Int64  `boil:"id_production_products_pallets" json:"id_production_products_pallets,omitempty" toml:"id_production_products_pallets" yaml:"id_production_products_pallets,omitempty"`
...
var ProductionProductsBoxWhere = struct {
	...
	IDProductionProductsPallets whereHelpernull_Int64
	BoxNumber                   whereHelpernull_String
}{
	ID:                          whereHelperint64{field: "\"production_products_boxes\".\"id\""},
	IDProductionProducts:        whereHelpernull_Int64{field: "\"production_products_boxes\".\"id_production_products\""},
	IDProductionProductsPallets: whereHelpernull_Int64{field: "\"production_products_boxes\".\"id_production_products_pallets\""},
	BoxNumber:                   whereHelpernull_String{field: "\"production_products_boxes\".\"box_number\""},
}
...
var (
	...
	productionProductsBoxColumnsWithoutDefault = []string{}
	productionProductsBoxColumnsWithDefault    = []string{"id", "id_production_products", "id_production_products_pallets", "box_number"}
...
func ProductionProductsBoxes(mods ...qm.QueryMod) productionProductsBoxQuery {
	mods = append(mods, qm.From("\"production_products_boxes\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"production_products_boxes\".*"})
	}
...
func FindProductionProductsBoxG(ctx context.Context, iD int64, selectCols ...string) (*ProductionProductsBox, error) {
...
func FindProductionProductsBox(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*ProductionProductsBox, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"production_products_boxes\" where \"id\"=?", sel,
	)
...
func (o *ProductionProductsBox) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"production_products_boxes\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"production_products_boxes\" %sDEFAULT VALUES%s"
		}
...
func (o *ProductionProductsBox) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"production_products_boxes\" SET %s WHERE %s",
			strmangle.SetParamNames("\"", "\"", 0, wl),
			strmangle.WhereClause("\"", "\"", 0, productionProductsBoxPrimaryKeyColumns),
		)
...
func (o ProductionProductsBoxSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"production_products_boxes\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 0, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, productionProductsBoxPrimaryKeyColumns, len(o)))
...
func (o *ProductionProductsBox) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *ProductionProductsBox) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 {
			conflict = make([]string, len(productionProductsBoxPrimaryKeyColumns))
			copy(conflict, productionProductsBoxPrimaryKeyColumns)
		}
		cache.query = buildUpsertQuerySQLite(dialect, "\"production_products_boxes\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(productionProductsBoxType, productionProductsBoxMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *ProductionProductsBox) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"production_products_boxes\" WHERE \"id\"=?"
...
func (o ProductionProductsBoxSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"production_products_boxes\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, productionProductsBoxPrimaryKeyColumns, len(o))
...
func (o *ProductionProductsBoxSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"production_products_boxes\".* FROM \"production_products_boxes\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, productionProductsBoxPrimaryKeyColumns, len(*o))
...
func ProductionProductsBoxExistsG(ctx context.Context, iD int64) (bool, error) {
...
func ProductionProductsBoxExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"production_products_boxes\" where \"id\"=? limit 1)"
