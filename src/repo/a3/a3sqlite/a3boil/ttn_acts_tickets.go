...
var TTNActsTicketWhere = struct {
	...
	ID:            whereHelperint64{field: "\"ttn_acts_tickets\".\"id\""},
	IDTTNActs:     whereHelpernull_Int64{field: "\"ttn_acts_tickets\".\"id_ttn_acts\""},
	TicketType:    whereHelpernull_String{field: "\"ttn_acts_tickets\".\"ticket_type\""},
	TicketNumber:  whereHelpernull_String{field: "\"ttn_acts_tickets\".\"ticket_number\""},
	TicketDate:    whereHelpernull_String{field: "\"ttn_acts_tickets\".\"ticket_date\""},
	TicketRegID:   whereHelpernull_String{field: "\"ttn_acts_tickets\".\"ticket_reg_id\""},
	TicketComment: whereHelpernull_String{field: "\"ttn_acts_tickets\".\"ticket_comment\""},
	Status:        whereHelpernull_String{field: "\"ttn_acts_tickets\".\"status\""},
	ReplyID:       whereHelpernull_String{field: "\"ttn_acts_tickets\".\"reply_id\""},
	XML:           whereHelpernull_String{field: "\"ttn_acts_tickets\".\"xml\""},
}
...
var (
	...
	ttnActsTicketColumnsWithoutDefault = []string{}
	ttnActsTicketColumnsWithDefault    = []string{"id", "id_ttn_acts", "ticket_type", "ticket_number", "ticket_date", "ticket_reg_id", "ticket_comment", "status", "reply_id", "xml"}
...
func TTNActsTickets(mods ...qm.QueryMod) ttnActsTicketQuery {
	mods = append(mods, qm.From("\"ttn_acts_tickets\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"ttn_acts_tickets\".*"})
	}
...
func FindTTNActsTicketG(ctx context.Context, iD int64, selectCols ...string) (*TTNActsTicket, error) {
...
func FindTTNActsTicket(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*TTNActsTicket, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"ttn_acts_tickets\" where \"id\"=?", sel,
	)
...
func (o *TTNActsTicket) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"ttn_acts_tickets\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"ttn_acts_tickets\" %sDEFAULT VALUES%s"
		}
...
func (o *TTNActsTicket) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"ttn_acts_tickets\" SET %s WHERE %s",
			strmangle.SetParamNames("\"", "\"", 0, wl),
			strmangle.WhereClause("\"", "\"", 0, ttnActsTicketPrimaryKeyColumns),
		)
...
func (o TTNActsTicketSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"ttn_acts_tickets\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 0, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, ttnActsTicketPrimaryKeyColumns, len(o)))
...
func (o *TTNActsTicket) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *TTNActsTicket) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 {
			conflict = make([]string, len(ttnActsTicketPrimaryKeyColumns))
			copy(conflict, ttnActsTicketPrimaryKeyColumns)
		}
		cache.query = buildUpsertQuerySQLite(dialect, "\"ttn_acts_tickets\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(ttnActsTicketType, ttnActsTicketMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *TTNActsTicket) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"ttn_acts_tickets\" WHERE \"id\"=?"
...
func (o TTNActsTicketSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"ttn_acts_tickets\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, ttnActsTicketPrimaryKeyColumns, len(o))
...
func (o *TTNActsTicketSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"ttn_acts_tickets\".* FROM \"ttn_acts_tickets\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, ttnActsTicketPrimaryKeyColumns, len(*o))
...
func TTNActsTicketExistsG(ctx context.Context, iD int64) (bool, error) {
	return TTNActsTicketExists(ctx, boil.GetContextDB(), iD)
}

// TTNActsTicketExists checks if the TTNActsTicket row exists.
func TTNActsTicketExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"ttn_acts_tickets\" where \"id\"=? limit 1)"
