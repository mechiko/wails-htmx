...
var ProductionProductsPalletWhere = struct {
	...
	ID:                   whereHelperint{field: "[dbo].[production_products_pallets].[id]"},
	IDProductionProducts: whereHelpernull_Int{field: "[dbo].[production_products_pallets].[id_production_products]"},
	PalletNumber:         whereHelpernull_String{field: "[dbo].[production_products_pallets].[pallet_number]"},
}
...
var (
	...
	productionProductsPalletColumnsWithoutDefault = []string{"id_production_products", "pallet_number"}
	productionProductsPalletColumnsWithDefault    = []string{"id"}
...
func ProductionProductsPallets(mods ...qm.QueryMod) productionProductsPalletQuery {
	mods = append(mods, qm.From("[dbo].[production_products_pallets]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[production_products_pallets].*"})
	}
...
func FindProductionProductsPalletG(ctx context.Context, iD int, selectCols ...string) (*ProductionProductsPallet, error) {
...
func FindProductionProductsPallet(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*ProductionProductsPallet, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[production_products_pallets] where [id]=$1", sel,
	)
...
func (o *ProductionProductsPallet) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[production_products_pallets] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[production_products_pallets] %sDEFAULT VALUES%s"
		}
...
func (o *ProductionProductsPallet) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[production_products_pallets] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, productionProductsPalletPrimaryKeyColumns),
		)
...
func (o ProductionProductsPalletSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[production_products_pallets] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, productionProductsPalletPrimaryKeyColumns, len(o)))
...
func (o *ProductionProductsPallet) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *ProductionProductsPallet) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, productionProductsPalletGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(productionProductsPalletPrimaryKeyColumns, v) && strmangle.ContainsAny(productionProductsPalletColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("a3boil: unable to upsert production_products_pallets, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(productionProductsPalletAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("a3boil: unable to upsert production_products_pallets, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[production_products_pallets]", productionProductsPalletPrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(productionProductsPalletPrimaryKeyColumns))
		copy(whitelist, productionProductsPalletPrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(productionProductsPalletType, productionProductsPalletMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *ProductionProductsPallet) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[production_products_pallets] WHERE [id]=$1"
...
func (o ProductionProductsPalletSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[production_products_pallets] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, productionProductsPalletPrimaryKeyColumns, len(o))
...
func (o *ProductionProductsPalletSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[production_products_pallets].* FROM [dbo].[production_products_pallets] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, productionProductsPalletPrimaryKeyColumns, len(*o))
...
func ProductionProductsPalletExistsG(ctx context.Context, iD int) (bool, error) {
...
func ProductionProductsPalletExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[production_products_pallets] where [id]=$1) then 1 else 0 end"
