...
type ProductionResourcesMark struct {
	...
	IDProductionResourcesBoxes null.Int    `boil:"id_production_resources_boxes" json:"id_production_resources_boxes,omitempty" toml:"id_production_resources_boxes" yaml:"id_production_resources_boxes,omitempty"`
...
var ProductionResourcesMarkWhere = struct {
	...
	IDProductionResourcesBoxes whereHelpernull_Int
	Mark                       whereHelpernull_String
}{
	ID:                         whereHelperint{field: "[dbo].[production_resources_marks].[id]"},
	IDProductionResources:      whereHelpernull_Int{field: "[dbo].[production_resources_marks].[id_production_resources]"},
	IDProductionResourcesBoxes: whereHelpernull_Int{field: "[dbo].[production_resources_marks].[id_production_resources_boxes]"},
	Mark:                       whereHelpernull_String{field: "[dbo].[production_resources_marks].[mark]"},
}
...
var (
	...
	productionResourcesMarkColumnsWithoutDefault = []string{"id_production_resources", "id_production_resources_boxes", "mark"}
	productionResourcesMarkColumnsWithDefault    = []string{"id"}
...
func ProductionResourcesMarks(mods ...qm.QueryMod) productionResourcesMarkQuery {
	mods = append(mods, qm.From("[dbo].[production_resources_marks]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[production_resources_marks].*"})
	}
...
func FindProductionResourcesMarkG(ctx context.Context, iD int, selectCols ...string) (*ProductionResourcesMark, error) {
...
func FindProductionResourcesMark(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*ProductionResourcesMark, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[production_resources_marks] where [id]=$1", sel,
	)
...
func (o *ProductionResourcesMark) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[production_resources_marks] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[production_resources_marks] %sDEFAULT VALUES%s"
		}
...
func (o *ProductionResourcesMark) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[production_resources_marks] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, productionResourcesMarkPrimaryKeyColumns),
		)
...
func (o ProductionResourcesMarkSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[production_resources_marks] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, productionResourcesMarkPrimaryKeyColumns, len(o)))
...
func (o *ProductionResourcesMark) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *ProductionResourcesMark) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, productionResourcesMarkGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(productionResourcesMarkPrimaryKeyColumns, v) && strmangle.ContainsAny(productionResourcesMarkColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("a3boil: unable to upsert production_resources_marks, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(productionResourcesMarkAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("a3boil: unable to upsert production_resources_marks, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[production_resources_marks]", productionResourcesMarkPrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(productionResourcesMarkPrimaryKeyColumns))
		copy(whitelist, productionResourcesMarkPrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(productionResourcesMarkType, productionResourcesMarkMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *ProductionResourcesMark) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[production_resources_marks] WHERE [id]=$1"
...
func (o ProductionResourcesMarkSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[production_resources_marks] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, productionResourcesMarkPrimaryKeyColumns, len(o))
...
func (o *ProductionResourcesMarkSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[production_resources_marks].* FROM [dbo].[production_resources_marks] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, productionResourcesMarkPrimaryKeyColumns, len(*o))
...
func ProductionResourcesMarkExistsG(ctx context.Context, iD int) (bool, error) {
...
func ProductionResourcesMarkExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[production_resources_marks] where [id]=$1) then 1 else 0 end"
