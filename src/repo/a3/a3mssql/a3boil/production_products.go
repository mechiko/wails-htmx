...
var ProductionProductWhere = struct {
	...
	ID:                  whereHelperint{field: "[dbo].[production_products].[id]"},
	IDProductionReports: whereHelpernull_Int{field: "[dbo].[production_products].[id_production_reports]"},
	ProductFullName:     whereHelpernull_String{field: "[dbo].[production_products].[product_full_name]"},
	ProductCapacity:     whereHelpernull_String{field: "[dbo].[production_products].[product_capacity]"},
	ProductAlcVolume:    whereHelpernull_String{field: "[dbo].[production_products].[product_alc_volume]"},
	ProductAlcVolumeMin: whereHelpernull_String{field: "[dbo].[production_products].[product_alc_volume_min]"},
	ProductAlcVolumeMax: whereHelpernull_String{field: "[dbo].[production_products].[product_alc_volume_max]"},
	ProductAlcCode:      whereHelpernull_String{field: "[dbo].[production_products].[product_alc_code]"},
	ProductCode:         whereHelpernull_String{field: "[dbo].[production_products].[product_code]"},
	ProductUnitType:     whereHelpernull_String{field: "[dbo].[production_products].[product_unit_type]"},
	ProductIddomain:     whereHelpernull_String{field: "[dbo].[production_products].[product_iddomain]"},
	ProductQuantity:     whereHelpernull_String{field: "[dbo].[production_products].[product_quantity]"},
	ProductParty:        whereHelpernull_String{field: "[dbo].[production_products].[product_party]"},
	ProductComment:      whereHelpernull_String{field: "[dbo].[production_products].[product_comment]"},
	ProducerType:        whereHelpernull_String{field: "[dbo].[production_products].[producer_type]"},
	ProducerClientRegID: whereHelpernull_String{field: "[dbo].[production_products].[producer_client_reg_id]"},
	ProducerInn:         whereHelpernull_String{field: "[dbo].[production_products].[producer_inn]"},
	ProducerKPP:         whereHelpernull_String{field: "[dbo].[production_products].[producer_kpp]"},
	ProducerFullName:    whereHelpernull_String{field: "[dbo].[production_products].[producer_full_name]"},
	ProducerShortName:   whereHelpernull_String{field: "[dbo].[production_products].[producer_short_name]"},
	ProducerCountryCode: whereHelpernull_String{field: "[dbo].[production_products].[producer_country_code]"},
	ProducerRegionCode:  whereHelpernull_String{field: "[dbo].[production_products].[producer_region_code]"},
	ProducerDescription: whereHelpernull_String{field: "[dbo].[production_products].[producer_description]"},
}
...
var (
	...
	productionProductColumnsWithoutDefault = []string{"id_production_reports", "product_full_name", "product_capacity", "product_alc_volume", "product_alc_volume_min", "product_alc_volume_max", "product_alc_code", "product_code", "product_unit_type", "product_iddomain", "product_quantity", "product_party", "product_comment", "producer_type", "producer_client_reg_id", "producer_inn", "producer_kpp", "producer_full_name", "producer_short_name", "producer_country_code", "producer_region_code", "producer_description"}
	productionProductColumnsWithDefault    = []string{"id"}
...
func ProductionProducts(mods ...qm.QueryMod) productionProductQuery {
	mods = append(mods, qm.From("[dbo].[production_products]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[production_products].*"})
	}
...
func FindProductionProductG(ctx context.Context, iD int, selectCols ...string) (*ProductionProduct, error) {
...
func FindProductionProduct(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*ProductionProduct, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[production_products] where [id]=$1", sel,
	)
...
func (o *ProductionProduct) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[production_products] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[production_products] %sDEFAULT VALUES%s"
		}
...
func (o *ProductionProduct) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[production_products] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, productionProductPrimaryKeyColumns),
		)
...
func (o ProductionProductSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[production_products] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, productionProductPrimaryKeyColumns, len(o)))
...
func (o *ProductionProduct) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *ProductionProduct) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, productionProductGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(productionProductPrimaryKeyColumns, v) && strmangle.ContainsAny(productionProductColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("a3boil: unable to upsert production_products, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(productionProductAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("a3boil: unable to upsert production_products, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[production_products]", productionProductPrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(productionProductPrimaryKeyColumns))
		copy(whitelist, productionProductPrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(productionProductType, productionProductMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *ProductionProduct) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[production_products] WHERE [id]=$1"
...
func (o ProductionProductSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[production_products] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, productionProductPrimaryKeyColumns, len(o))
...
func (o *ProductionProductSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[production_products].* FROM [dbo].[production_products] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, productionProductPrimaryKeyColumns, len(*o))
...
func ProductionProductExistsG(ctx context.Context, iD int) (bool, error) {
...
func ProductionProductExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[production_products] where [id]=$1) then 1 else 0 end"
