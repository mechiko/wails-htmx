...
type TN struct {
	...
	DocOwnership         null.Int    `boil:"doc_ownership" json:"doc_ownership,omitempty" toml:"doc_ownership" yaml:"doc_ownership,omitempty"`
...
var TNWhere = struct {
	...
	DocOwnership         whereHelpernull_Int
	...
	ID:                   whereHelperint{field: "[dbo].[tn].[id]"},
	IDTTN:                whereHelpernull_Int{field: "[dbo].[tn].[id_ttn]"},
	CreateDate:           whereHelpernull_String{field: "[dbo].[tn].[create_date]"},
	DocNumber:            whereHelpernull_String{field: "[dbo].[tn].[doc_number]"},
	DocDate:              whereHelpernull_String{field: "[dbo].[tn].[doc_date]"},
	DocQuantity:          whereHelpernull_String{field: "[dbo].[tn].[doc_quantity]"},
	DocOwnership:         whereHelpernull_Int{field: "[dbo].[tn].[doc_ownership]"},
	DocRegID:             whereHelpernull_String{field: "[dbo].[tn].[doc_reg_id]"},
	TranType:             whereHelpernull_String{field: "[dbo].[tn].[tran_type]"},
	TransportCompany:     whereHelpernull_String{field: "[dbo].[tn].[transport_company]"},
	TransportCar:         whereHelpernull_String{field: "[dbo].[tn].[transport_car]"},
	TransportCustomer:    whereHelpernull_String{field: "[dbo].[tn].[transport_customer]"},
	TransportDriver:      whereHelpernull_String{field: "[dbo].[tn].[transport_driver]"},
	TransportTrailer:     whereHelpernull_String{field: "[dbo].[tn].[transport_trailer]"},
	TransportForwarder:   whereHelpernull_String{field: "[dbo].[tn].[transport_forwarder]"},
	TransportLoadPoint:   whereHelpernull_String{field: "[dbo].[tn].[transport_load_point]"},
	TransportUnloadPoint: whereHelpernull_String{field: "[dbo].[tn].[transport_unload_point]"},
	TransportRedirect:    whereHelpernull_String{field: "[dbo].[tn].[transport_redirect]"},
	Version:              whereHelpernull_String{field: "[dbo].[tn].[version]"},
	State:                whereHelpernull_String{field: "[dbo].[tn].[state]"},
	Status:               whereHelpernull_String{field: "[dbo].[tn].[status]"},
	ReplyID:              whereHelpernull_String{field: "[dbo].[tn].[reply_id]"},
	XML:                  whereHelpernull_String{field: "[dbo].[tn].[xml]"},
}
...
var (
	...
	tnColumnsWithoutDefault = []string{"id_ttn", "create_date", "doc_number", "doc_date", "doc_quantity", "doc_ownership", "doc_reg_id", "tran_type", "transport_company", "transport_car", "transport_customer", "transport_driver", "transport_trailer", "transport_forwarder", "transport_load_point", "transport_unload_point", "transport_redirect", "version", "state", "status", "reply_id", "xml"}
	tnColumnsWithDefault    = []string{"id"}
...
func TNS(mods ...qm.QueryMod) tnQuery {
	mods = append(mods, qm.From("[dbo].[tn]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[tn].*"})
	}
...
func FindTNG(ctx context.Context, iD int, selectCols ...string) (*TN, error) {
	return FindTN(ctx, boil.GetContextDB(), iD, selectCols...)
}

// FindTN retrieves a single record by ID with an executor.
// If selectCols is empty Find will return all columns.
func FindTN(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*TN, error) {
	tnObj := &TN{}
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[tn] where [id]=$1", sel,
	)
...
func (o *TN) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[tn] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[tn] %sDEFAULT VALUES%s"
		}
...
func (o *TN) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[tn] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, tnPrimaryKeyColumns),
		)
...
func (o TNSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[tn] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, tnPrimaryKeyColumns, len(o)))
...
func (o *TN) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *TN) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, tnGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(tnPrimaryKeyColumns, v) && strmangle.ContainsAny(tnColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("a3boil: unable to upsert tn, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(tnAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("a3boil: unable to upsert tn, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[tn]", tnPrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(tnPrimaryKeyColumns))
		copy(whitelist, tnPrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(tnType, tnMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *TN) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[tn] WHERE [id]=$1"
...
func (o TNSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[tn] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, tnPrimaryKeyColumns, len(o))
...
func (o *TNSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[tn].* FROM [dbo].[tn] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, tnPrimaryKeyColumns, len(*o))
...
func TNExistsG(ctx context.Context, iD int) (bool, error) {
	return TNExists(ctx, boil.GetContextDB(), iD)
}

// TNExists checks if the TN row exists.
func TNExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[tn] where [id]=$1) then 1 else 0 end"
