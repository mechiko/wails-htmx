...
var DeclarationCompanyWhere = struct {
	...
	ID:         whereHelperint{field: "[dbo].[declaration_companies].[id]"},
	Type:       whereHelpernull_String{field: "[dbo].[declaration_companies].[type]"},
	FullName:   whereHelpernull_String{field: "[dbo].[declaration_companies].[full_name]"},
	Inn:        whereHelpernull_String{field: "[dbo].[declaration_companies].[inn]"},
	KPP:        whereHelpernull_String{field: "[dbo].[declaration_companies].[kpp]"},
	PostIndex:  whereHelpernull_String{field: "[dbo].[declaration_companies].[post_index]"},
	RegionCode: whereHelpernull_String{field: "[dbo].[declaration_companies].[region_code]"},
	District:   whereHelpernull_String{field: "[dbo].[declaration_companies].[district]"},
	City:       whereHelpernull_String{field: "[dbo].[declaration_companies].[city]"},
	Locality:   whereHelpernull_String{field: "[dbo].[declaration_companies].[locality]"},
	Street:     whereHelpernull_String{field: "[dbo].[declaration_companies].[street]"},
	House:      whereHelpernull_String{field: "[dbo].[declaration_companies].[house]"},
	Building:   whereHelpernull_String{field: "[dbo].[declaration_companies].[building]"},
	Letter:     whereHelpernull_String{field: "[dbo].[declaration_companies].[letter]"},
	Apartment:  whereHelpernull_String{field: "[dbo].[declaration_companies].[apartment]"},
	Phone:      whereHelpernull_String{field: "[dbo].[declaration_companies].[phone]"},
	Email:      whereHelpernull_String{field: "[dbo].[declaration_companies].[email]"},
}
...
var (
	...
	declarationCompanyColumnsWithoutDefault = []string{"type", "full_name", "inn", "kpp", "post_index", "region_code", "district", "city", "locality", "street", "house", "building", "letter", "apartment", "phone", "email"}
	declarationCompanyColumnsWithDefault    = []string{"id"}
...
func DeclarationCompanies(mods ...qm.QueryMod) declarationCompanyQuery {
	mods = append(mods, qm.From("[dbo].[declaration_companies]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[declaration_companies].*"})
	}
...
func FindDeclarationCompanyG(ctx context.Context, iD int, selectCols ...string) (*DeclarationCompany, error) {
...
func FindDeclarationCompany(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*DeclarationCompany, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[declaration_companies] where [id]=$1", sel,
	)
...
func (o *DeclarationCompany) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[declaration_companies] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[declaration_companies] %sDEFAULT VALUES%s"
		}
...
func (o *DeclarationCompany) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[declaration_companies] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, declarationCompanyPrimaryKeyColumns),
		)
...
func (o DeclarationCompanySlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[declaration_companies] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, declarationCompanyPrimaryKeyColumns, len(o)))
...
func (o *DeclarationCompany) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *DeclarationCompany) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, declarationCompanyGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(declarationCompanyPrimaryKeyColumns, v) && strmangle.ContainsAny(declarationCompanyColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("a3boil: unable to upsert declaration_companies, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(declarationCompanyAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("a3boil: unable to upsert declaration_companies, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[declaration_companies]", declarationCompanyPrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(declarationCompanyPrimaryKeyColumns))
		copy(whitelist, declarationCompanyPrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(declarationCompanyType, declarationCompanyMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *DeclarationCompany) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[declaration_companies] WHERE [id]=$1"
...
func (o DeclarationCompanySlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[declaration_companies] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, declarationCompanyPrimaryKeyColumns, len(o))
...
func (o *DeclarationCompanySlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[declaration_companies].* FROM [dbo].[declaration_companies] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, declarationCompanyPrimaryKeyColumns, len(*o))
...
func DeclarationCompanyExistsG(ctx context.Context, iD int) (bool, error) {
...
func DeclarationCompanyExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[declaration_companies] where [id]=$1) then 1 else 0 end"
