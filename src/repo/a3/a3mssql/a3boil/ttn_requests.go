...
var TTNRequestWhere = struct {
	...
	ID:            whereHelperint{field: "[dbo].[ttn_requests].[id]"},
	IDTTN:         whereHelpernull_Int{field: "[dbo].[ttn_requests].[id_ttn]"},
	ClientRegID:   whereHelpernull_String{field: "[dbo].[ttn_requests].[client_reg_id]"},
	RequestNumber: whereHelpernull_String{field: "[dbo].[ttn_requests].[request_number]"},
	RequestDate:   whereHelpernull_String{field: "[dbo].[ttn_requests].[request_date]"},
	RequestRegID:  whereHelpernull_String{field: "[dbo].[ttn_requests].[request_reg_id]"},
	Status:        whereHelpernull_String{field: "[dbo].[ttn_requests].[status]"},
	ReplyID:       whereHelpernull_String{field: "[dbo].[ttn_requests].[reply_id]"},
}
...
var (
	...
	ttnRequestColumnsWithoutDefault = []string{"id_ttn", "client_reg_id", "request_number", "request_date", "request_reg_id", "status", "reply_id"}
	ttnRequestColumnsWithDefault    = []string{"id"}
...
func TTNRequests(mods ...qm.QueryMod) ttnRequestQuery {
	mods = append(mods, qm.From("[dbo].[ttn_requests]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[ttn_requests].*"})
	}
...
func FindTTNRequestG(ctx context.Context, iD int, selectCols ...string) (*TTNRequest, error) {
...
func FindTTNRequest(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*TTNRequest, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[ttn_requests] where [id]=$1", sel,
	)
...
func (o *TTNRequest) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[ttn_requests] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[ttn_requests] %sDEFAULT VALUES%s"
		}
...
func (o *TTNRequest) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[ttn_requests] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, ttnRequestPrimaryKeyColumns),
		)
...
func (o TTNRequestSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[ttn_requests] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, ttnRequestPrimaryKeyColumns, len(o)))
...
func (o *TTNRequest) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *TTNRequest) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, ttnRequestGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(ttnRequestPrimaryKeyColumns, v) && strmangle.ContainsAny(ttnRequestColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("a3boil: unable to upsert ttn_requests, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(ttnRequestAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("a3boil: unable to upsert ttn_requests, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[ttn_requests]", ttnRequestPrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(ttnRequestPrimaryKeyColumns))
		copy(whitelist, ttnRequestPrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(ttnRequestType, ttnRequestMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *TTNRequest) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[ttn_requests] WHERE [id]=$1"
...
func (o TTNRequestSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[ttn_requests] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, ttnRequestPrimaryKeyColumns, len(o))
...
func (o *TTNRequestSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[ttn_requests].* FROM [dbo].[ttn_requests] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, ttnRequestPrimaryKeyColumns, len(*o))
...
func TTNRequestExistsG(ctx context.Context, iD int) (bool, error) {
	return TTNRequestExists(ctx, boil.GetContextDB(), iD)
}

// TTNRequestExists checks if the TTNRequest row exists.
func TTNRequestExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[ttn_requests] where [id]=$1) then 1 else 0 end"
