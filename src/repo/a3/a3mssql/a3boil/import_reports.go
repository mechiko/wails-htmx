...
var ImportReportWhere = struct {
	...
	ContractNumber:      whereHelpernull_String{field: "[dbo].[import_reports].[contract_number]"},
	ContractDate:        whereHelpernull_String{field: "[dbo].[import_reports].[contract_date]"},
	GTDNumber:           whereHelpernull_String{field: "[dbo].[import_reports].[gtd_number]"},
	GTDDate:             whereHelpernull_String{field: "[dbo].[import_reports].[gtd_date]"},
	DocComment:          whereHelpernull_String{field: "[dbo].[import_reports].[doc_comment]"},
	ImporterType:        whereHelpernull_String{field: "[dbo].[import_reports].[importer_type]"},
	ImporterClientRegID: whereHelpernull_String{field: "[dbo].[import_reports].[importer_client_reg_id]"},
	ImporterInn:         whereHelpernull_String{field: "[dbo].[import_reports].[importer_inn]"},
	ImporterKPP:         whereHelpernull_String{field: "[dbo].[import_reports].[importer_kpp]"},
	ImporterFullName:    whereHelpernull_String{field: "[dbo].[import_reports].[importer_full_name]"},
	ImporterShortName:   whereHelpernull_String{field: "[dbo].[import_reports].[importer_short_name]"},
	ImporterCountryCode: whereHelpernull_String{field: "[dbo].[import_reports].[importer_country_code]"},
	ImporterRegionCode:  whereHelpernull_String{field: "[dbo].[import_reports].[importer_region_code]"},
	ImporterDescription: whereHelpernull_String{field: "[dbo].[import_reports].[importer_description]"},
	SupplierType:        whereHelpernull_String{field: "[dbo].[import_reports].[supplier_type]"},
	SupplierClientRegID: whereHelpernull_String{field: "[dbo].[import_reports].[supplier_client_reg_id]"},
	SupplierInn:         whereHelpernull_String{field: "[dbo].[import_reports].[supplier_inn]"},
	SupplierKPP:         whereHelpernull_String{field: "[dbo].[import_reports].[supplier_kpp]"},
	SupplierFullName:    whereHelpernull_String{field: "[dbo].[import_reports].[supplier_full_name]"},
	SupplierShortName:   whereHelpernull_String{field: "[dbo].[import_reports].[supplier_short_name]"},
	SupplierCountryCode: whereHelpernull_String{field: "[dbo].[import_reports].[supplier_country_code]"},
	SupplierRegionCode:  whereHelpernull_String{field: "[dbo].[import_reports].[supplier_region_code]"},
	SupplierDescription: whereHelpernull_String{field: "[dbo].[import_reports].[supplier_description]"},
	Version:             whereHelpernull_String{field: "[dbo].[import_reports].[version]"},
	State:               whereHelpernull_String{field: "[dbo].[import_reports].[state]"},
	Status:              whereHelpernull_String{field: "[dbo].[import_reports].[status]"},
	ReplyID:             whereHelpernull_String{field: "[dbo].[import_reports].[reply_id]"},
	Archive:             whereHelpernull_Int{field: "[dbo].[import_reports].[archive]"},
	XML:                 whereHelpernull_String{field: "[dbo].[import_reports].[xml]"},
}
...
func ImportReports(mods ...qm.QueryMod) importReportQuery {
	mods = append(mods, qm.From("[dbo].[import_reports]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[import_reports].*"})
	}
...
func FindImportReportG(ctx context.Context, iD int, selectCols ...string) (*ImportReport, error) {
...
func FindImportReport(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*ImportReport, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[import_reports] where [id]=$1", sel,
	)
...
func (o *ImportReport) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[import_reports] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[import_reports] %sDEFAULT VALUES%s"
		}
...
func (o *ImportReport) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[import_reports] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, importReportPrimaryKeyColumns),
		)
...
func (o ImportReportSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[import_reports] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, importReportPrimaryKeyColumns, len(o)))
...
func (o *ImportReport) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *ImportReport) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, importReportGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(importReportPrimaryKeyColumns, v) && strmangle.ContainsAny(importReportColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("a3boil: unable to upsert import_reports, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(importReportAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("a3boil: unable to upsert import_reports, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[import_reports]", importReportPrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(importReportPrimaryKeyColumns))
		copy(whitelist, importReportPrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(importReportType, importReportMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *ImportReport) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[import_reports] WHERE [id]=$1"
...
func (o ImportReportSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[import_reports] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, importReportPrimaryKeyColumns, len(o))
...
func (o *ImportReportSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[import_reports].* FROM [dbo].[import_reports] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, importReportPrimaryKeyColumns, len(*o))
...
func ImportReportExistsG(ctx context.Context, iD int) (bool, error) {
	return ImportReportExists(ctx, boil.GetContextDB(), iD)
}

// ImportReportExists checks if the ImportReport row exists.
func ImportReportExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[import_reports] where [id]=$1) then 1 else 0 end"
