...
type ChargeOnProductsMark struct {
	...
	IDChargeOnProducts null.Int    `boil:"id_charge_on_products" json:"id_charge_on_products,omitempty" toml:"id_charge_on_products" yaml:"id_charge_on_products,omitempty"`
...
var ChargeOnProductsMarkWhere = struct {
	ID                 whereHelperint
	IDChargeOnProducts whereHelpernull_Int
	Mark               whereHelpernull_String
}{
	ID:                 whereHelperint{field: "[dbo].[charge_on_products_marks].[id]"},
	IDChargeOnProducts: whereHelpernull_Int{field: "[dbo].[charge_on_products_marks].[id_charge_on_products]"},
	Mark:               whereHelpernull_String{field: "[dbo].[charge_on_products_marks].[mark]"},
}
...
var (
	...
	chargeOnProductsMarkColumnsWithoutDefault = []string{"id_charge_on_products", "mark"}
	chargeOnProductsMarkColumnsWithDefault    = []string{"id"}
...
func ChargeOnProductsMarks(mods ...qm.QueryMod) chargeOnProductsMarkQuery {
	mods = append(mods, qm.From("[dbo].[charge_on_products_marks]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[charge_on_products_marks].*"})
	}
...
func FindChargeOnProductsMarkG(ctx context.Context, iD int, selectCols ...string) (*ChargeOnProductsMark, error) {
...
func FindChargeOnProductsMark(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*ChargeOnProductsMark, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[charge_on_products_marks] where [id]=$1", sel,
	)
...
func (o *ChargeOnProductsMark) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[charge_on_products_marks] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[charge_on_products_marks] %sDEFAULT VALUES%s"
		}
...
func (o *ChargeOnProductsMark) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[charge_on_products_marks] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, chargeOnProductsMarkPrimaryKeyColumns),
		)
...
func (o ChargeOnProductsMarkSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[charge_on_products_marks] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, chargeOnProductsMarkPrimaryKeyColumns, len(o)))
...
func (o *ChargeOnProductsMark) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *ChargeOnProductsMark) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, chargeOnProductsMarkGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(chargeOnProductsMarkPrimaryKeyColumns, v) && strmangle.ContainsAny(chargeOnProductsMarkColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("a3boil: unable to upsert charge_on_products_marks, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(chargeOnProductsMarkAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("a3boil: unable to upsert charge_on_products_marks, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[charge_on_products_marks]", chargeOnProductsMarkPrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(chargeOnProductsMarkPrimaryKeyColumns))
		copy(whitelist, chargeOnProductsMarkPrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(chargeOnProductsMarkType, chargeOnProductsMarkMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *ChargeOnProductsMark) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[charge_on_products_marks] WHERE [id]=$1"
...
func (o ChargeOnProductsMarkSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[charge_on_products_marks] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, chargeOnProductsMarkPrimaryKeyColumns, len(o))
...
func (o *ChargeOnProductsMarkSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[charge_on_products_marks].* FROM [dbo].[charge_on_products_marks] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, chargeOnProductsMarkPrimaryKeyColumns, len(*o))
...
func ChargeOnProductsMarkExistsG(ctx context.Context, iD int) (bool, error) {
...
func ChargeOnProductsMarkExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[charge_on_products_marks] where [id]=$1) then 1 else 0 end"
