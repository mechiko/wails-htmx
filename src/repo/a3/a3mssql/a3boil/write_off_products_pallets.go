...
var WriteOffProductsPalletWhere = struct {
	...
	ID:                 whereHelperint{field: "[dbo].[write_off_products_pallets].[id]"},
	IDWriteOffProducts: whereHelpernull_Int{field: "[dbo].[write_off_products_pallets].[id_write_off_products]"},
	PalletNumber:       whereHelpernull_String{field: "[dbo].[write_off_products_pallets].[pallet_number]"},
}
...
var (
	...
	writeOffProductsPalletColumnsWithoutDefault = []string{"id_write_off_products", "pallet_number"}
	writeOffProductsPalletColumnsWithDefault    = []string{"id"}
...
func WriteOffProductsPallets(mods ...qm.QueryMod) writeOffProductsPalletQuery {
	mods = append(mods, qm.From("[dbo].[write_off_products_pallets]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[write_off_products_pallets].*"})
	}
...
func FindWriteOffProductsPalletG(ctx context.Context, iD int, selectCols ...string) (*WriteOffProductsPallet, error) {
...
func FindWriteOffProductsPallet(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*WriteOffProductsPallet, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[write_off_products_pallets] where [id]=$1", sel,
	)
...
func (o *WriteOffProductsPallet) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[write_off_products_pallets] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[write_off_products_pallets] %sDEFAULT VALUES%s"
		}
...
func (o *WriteOffProductsPallet) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[write_off_products_pallets] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, writeOffProductsPalletPrimaryKeyColumns),
		)
...
func (o WriteOffProductsPalletSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[write_off_products_pallets] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, writeOffProductsPalletPrimaryKeyColumns, len(o)))
...
func (o *WriteOffProductsPallet) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *WriteOffProductsPallet) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, writeOffProductsPalletGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(writeOffProductsPalletPrimaryKeyColumns, v) && strmangle.ContainsAny(writeOffProductsPalletColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("a3boil: unable to upsert write_off_products_pallets, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(writeOffProductsPalletAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("a3boil: unable to upsert write_off_products_pallets, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[write_off_products_pallets]", writeOffProductsPalletPrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(writeOffProductsPalletPrimaryKeyColumns))
		copy(whitelist, writeOffProductsPalletPrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(writeOffProductsPalletType, writeOffProductsPalletMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *WriteOffProductsPallet) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[write_off_products_pallets] WHERE [id]=$1"
...
func (o WriteOffProductsPalletSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[write_off_products_pallets] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, writeOffProductsPalletPrimaryKeyColumns, len(o))
...
func (o *WriteOffProductsPalletSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[write_off_products_pallets].* FROM [dbo].[write_off_products_pallets] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, writeOffProductsPalletPrimaryKeyColumns, len(*o))
...
func WriteOffProductsPalletExistsG(ctx context.Context, iD int) (bool, error) {
...
func WriteOffProductsPalletExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[write_off_products_pallets] where [id]=$1) then 1 else 0 end"
