...
var ChargeOnProductWhere = struct {
	...
	ID:                  whereHelperint{field: "[dbo].[charge_on_products].[id]"},
	IDChargeOnActs:      whereHelpernull_Int{field: "[dbo].[charge_on_products].[id_charge_on_acts]"},
	ProductFullName:     whereHelpernull_String{field: "[dbo].[charge_on_products].[product_full_name]"},
	ProductCapacity:     whereHelpernull_String{field: "[dbo].[charge_on_products].[product_capacity]"},
	ProductAlcVolume:    whereHelpernull_String{field: "[dbo].[charge_on_products].[product_alc_volume]"},
	...
	ProducerInn:         whereHelpernull_String{field: "[dbo].[charge_on_products].[producer_inn]"},
	ProducerKPP:         whereHelpernull_String{field: "[dbo].[charge_on_products].[producer_kpp]"},
	ProducerFullName:    whereHelpernull_String{field: "[dbo].[charge_on_products].[producer_full_name]"},
	ProducerShortName:   whereHelpernull_String{field: "[dbo].[charge_on_products].[producer_short_name]"},
	ProducerCountryCode: whereHelpernull_String{field: "[dbo].[charge_on_products].[producer_country_code]"},
	ProducerRegionCode:  whereHelpernull_String{field: "[dbo].[charge_on_products].[producer_region_code]"},
	ProducerDescription: whereHelpernull_String{field: "[dbo].[charge_on_products].[producer_description]"},
	Form1Quantity:       whereHelpernull_String{field: "[dbo].[charge_on_products].[form1_quantity]"},
	Form1BottlingDate:   whereHelpernull_String{field: "[dbo].[charge_on_products].[form1_bottling_date]"},
	Form1TTNNumber:      whereHelpernull_String{field: "[dbo].[charge_on_products].[form1_ttn_number]"},
	Form1TTNDate:        whereHelpernull_String{field: "[dbo].[charge_on_products].[form1_ttn_date]"},
	Form1EgaisFixNumber: whereHelpernull_String{field: "[dbo].[charge_on_products].[form1_egais_fix_number]"},
	Form1EgaisFixDate:   whereHelpernull_String{field: "[dbo].[charge_on_products].[form1_egais_fix_date]"},
}
...
func ChargeOnProducts(mods ...qm.QueryMod) chargeOnProductQuery {
	mods = append(mods, qm.From("[dbo].[charge_on_products]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[charge_on_products].*"})
	}
...
func FindChargeOnProductG(ctx context.Context, iD int, selectCols ...string) (*ChargeOnProduct, error) {
...
func FindChargeOnProduct(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*ChargeOnProduct, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[charge_on_products] where [id]=$1", sel,
	)
...
func (o *ChargeOnProduct) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[charge_on_products] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[charge_on_products] %sDEFAULT VALUES%s"
		}
...
func (o *ChargeOnProduct) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[charge_on_products] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, chargeOnProductPrimaryKeyColumns),
		)
...
func (o ChargeOnProductSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[charge_on_products] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, chargeOnProductPrimaryKeyColumns, len(o)))
...
func (o *ChargeOnProduct) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *ChargeOnProduct) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, chargeOnProductGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(chargeOnProductPrimaryKeyColumns, v) && strmangle.ContainsAny(chargeOnProductColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("a3boil: unable to upsert charge_on_products, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(chargeOnProductAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("a3boil: unable to upsert charge_on_products, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[charge_on_products]", chargeOnProductPrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(chargeOnProductPrimaryKeyColumns))
		copy(whitelist, chargeOnProductPrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(chargeOnProductType, chargeOnProductMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *ChargeOnProduct) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[charge_on_products] WHERE [id]=$1"
...
func (o ChargeOnProductSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[charge_on_products] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, chargeOnProductPrimaryKeyColumns, len(o))
...
func (o *ChargeOnProductSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[charge_on_products].* FROM [dbo].[charge_on_products] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, chargeOnProductPrimaryKeyColumns, len(*o))
...
func ChargeOnProductExistsG(ctx context.Context, iD int) (bool, error) {
	return ChargeOnProductExists(ctx, boil.GetContextDB(), iD)
}

// ChargeOnProductExists checks if the ChargeOnProduct row exists.
func ChargeOnProductExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[charge_on_products] where [id]=$1) then 1 else 0 end"
