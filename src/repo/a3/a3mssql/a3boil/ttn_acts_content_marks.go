...
type TTNActsContentMark struct {
	...
	IDTTNActsContent null.Int    `boil:"id_ttn_acts_content" json:"id_ttn_acts_content,omitempty" toml:"id_ttn_acts_content" yaml:"id_ttn_acts_content,omitempty"`
...
var TTNActsContentMarkWhere = struct {
	ID               whereHelperint
	IDTTNActsContent whereHelpernull_Int
	Mark             whereHelpernull_String
}{
	ID:               whereHelperint{field: "[dbo].[ttn_acts_content_marks].[id]"},
	IDTTNActsContent: whereHelpernull_Int{field: "[dbo].[ttn_acts_content_marks].[id_ttn_acts_content]"},
	Mark:             whereHelpernull_String{field: "[dbo].[ttn_acts_content_marks].[mark]"},
}
...
var (
	...
	ttnActsContentMarkColumnsWithoutDefault = []string{"id_ttn_acts_content", "mark"}
	ttnActsContentMarkColumnsWithDefault    = []string{"id"}
...
func TTNActsContentMarks(mods ...qm.QueryMod) ttnActsContentMarkQuery {
	mods = append(mods, qm.From("[dbo].[ttn_acts_content_marks]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[ttn_acts_content_marks].*"})
	}
...
func FindTTNActsContentMarkG(ctx context.Context, iD int, selectCols ...string) (*TTNActsContentMark, error) {
...
func FindTTNActsContentMark(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*TTNActsContentMark, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[ttn_acts_content_marks] where [id]=$1", sel,
	)
...
func (o *TTNActsContentMark) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[ttn_acts_content_marks] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[ttn_acts_content_marks] %sDEFAULT VALUES%s"
		}
...
func (o *TTNActsContentMark) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[ttn_acts_content_marks] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, ttnActsContentMarkPrimaryKeyColumns),
		)
...
func (o TTNActsContentMarkSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[ttn_acts_content_marks] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, ttnActsContentMarkPrimaryKeyColumns, len(o)))
...
func (o *TTNActsContentMark) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *TTNActsContentMark) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, ttnActsContentMarkGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(ttnActsContentMarkPrimaryKeyColumns, v) && strmangle.ContainsAny(ttnActsContentMarkColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("a3boil: unable to upsert ttn_acts_content_marks, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(ttnActsContentMarkAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("a3boil: unable to upsert ttn_acts_content_marks, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[ttn_acts_content_marks]", ttnActsContentMarkPrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(ttnActsContentMarkPrimaryKeyColumns))
		copy(whitelist, ttnActsContentMarkPrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(ttnActsContentMarkType, ttnActsContentMarkMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *TTNActsContentMark) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[ttn_acts_content_marks] WHERE [id]=$1"
...
func (o TTNActsContentMarkSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[ttn_acts_content_marks] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, ttnActsContentMarkPrimaryKeyColumns, len(o))
...
func (o *TTNActsContentMarkSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[ttn_acts_content_marks].* FROM [dbo].[ttn_acts_content_marks] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, ttnActsContentMarkPrimaryKeyColumns, len(*o))
...
func TTNActsContentMarkExistsG(ctx context.Context, iD int) (bool, error) {
...
func TTNActsContentMarkExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[ttn_acts_content_marks] where [id]=$1) then 1 else 0 end"
