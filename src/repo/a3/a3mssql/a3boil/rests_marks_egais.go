...
var RestsMarksEgaisWhere = struct {
	...
	ID:             whereHelperint{field: "[dbo].[rests_marks_egais].[id]"},
	IDRequests:     whereHelpernull_Int{field: "[dbo].[rests_marks_egais].[id_requests]"},
	Mark:           whereHelpernull_String{field: "[dbo].[rests_marks_egais].[mark]"},
	InformF2RegID:  whereHelpernull_String{field: "[dbo].[rests_marks_egais].[inform_f2_reg_id]"},
	RestsMarksDate: whereHelpernull_String{field: "[dbo].[rests_marks_egais].[rests_marks_date]"},
}
...
var (
	...
	restsMarksEgaisColumnsWithoutDefault = []string{"id_requests", "mark", "inform_f2_reg_id", "rests_marks_date"}
	restsMarksEgaisColumnsWithDefault    = []string{"id"}
...
func RestsMarksEgaiss(mods ...qm.QueryMod) restsMarksEgaisQuery {
	mods = append(mods, qm.From("[dbo].[rests_marks_egais]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[rests_marks_egais].*"})
	}
...
func FindRestsMarksEgaisG(ctx context.Context, iD int, selectCols ...string) (*RestsMarksEgais, error) {
...
func FindRestsMarksEgais(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*RestsMarksEgais, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[rests_marks_egais] where [id]=$1", sel,
	)
...
func (o *RestsMarksEgais) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[rests_marks_egais] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[rests_marks_egais] %sDEFAULT VALUES%s"
		}
...
func (o *RestsMarksEgais) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[rests_marks_egais] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, restsMarksEgaisPrimaryKeyColumns),
		)
...
func (o RestsMarksEgaisSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[rests_marks_egais] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, restsMarksEgaisPrimaryKeyColumns, len(o)))
...
func (o *RestsMarksEgais) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *RestsMarksEgais) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, restsMarksEgaisGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(restsMarksEgaisPrimaryKeyColumns, v) && strmangle.ContainsAny(restsMarksEgaisColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("a3boil: unable to upsert rests_marks_egais, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(restsMarksEgaisAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("a3boil: unable to upsert rests_marks_egais, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[rests_marks_egais]", restsMarksEgaisPrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(restsMarksEgaisPrimaryKeyColumns))
		copy(whitelist, restsMarksEgaisPrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(restsMarksEgaisType, restsMarksEgaisMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *RestsMarksEgais) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[rests_marks_egais] WHERE [id]=$1"
...
func (o RestsMarksEgaisSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[rests_marks_egais] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, restsMarksEgaisPrimaryKeyColumns, len(o))
...
func (o *RestsMarksEgaisSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[rests_marks_egais].* FROM [dbo].[rests_marks_egais] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, restsMarksEgaisPrimaryKeyColumns, len(*o))
...
func RestsMarksEgaisExistsG(ctx context.Context, iD int) (bool, error) {
	return RestsMarksEgaisExists(ctx, boil.GetContextDB(), iD)
}

// RestsMarksEgaisExists checks if the RestsMarksEgais row exists.
func RestsMarksEgaisExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[rests_marks_egais] where [id]=$1) then 1 else 0 end"
