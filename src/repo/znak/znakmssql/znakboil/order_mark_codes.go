...
type OrderMarkCode struct {
	...
	TemplateID          null.Int    `boil:"template_id" json:"template_id,omitempty" toml:"template_id" yaml:"template_id,omitempty"`
...
var OrderMarkCodeWhere = struct {
	...
	TemplateID          whereHelpernull_Int
	...
	ID:                  whereHelperint{field: "[dbo].[order_mark_codes].[id]"},
	CreateDate:          whereHelpernull_String{field: "[dbo].[order_mark_codes].[create_date]"},
	Gtin:                whereHelpernull_String{field: "[dbo].[order_mark_codes].[gtin]"},
	Quantity:            whereHelpernull_Int{field: "[dbo].[order_mark_codes].[quantity]"},
	SerialNumberType:    whereHelpernull_String{field: "[dbo].[order_mark_codes].[serial_number_type]"},
	TemplateID:          whereHelpernull_Int{field: "[dbo].[order_mark_codes].[template_id]"},
	CisType:             whereHelpernull_String{field: "[dbo].[order_mark_codes].[cis_type]"},
	ContactPerson:       whereHelpernull_String{field: "[dbo].[order_mark_codes].[contact_person]"},
	ReleaseMethodType:   whereHelpernull_String{field: "[dbo].[order_mark_codes].[release_method_type]"},
	CreateMethodType:    whereHelpernull_String{field: "[dbo].[order_mark_codes].[create_method_type]"},
	PaymentType:         whereHelpernull_String{field: "[dbo].[order_mark_codes].[payment_type]"},
	ProductionOrderID:   whereHelpernull_String{field: "[dbo].[order_mark_codes].[production_order_id]"},
	ProductName:         whereHelpernull_String{field: "[dbo].[order_mark_codes].[product_name]"},
	ProductCapacity:     whereHelpernull_String{field: "[dbo].[order_mark_codes].[product_capacity]"},
	ProductShelfLife:    whereHelpernull_String{field: "[dbo].[order_mark_codes].[product_shelf_life]"},
	ProductTemplate:     whereHelpernull_String{field: "[dbo].[order_mark_codes].[product_template]"},
	ServiceProviderID:   whereHelpernull_String{field: "[dbo].[order_mark_codes].[service_provider_id]"},
	ServiceProviderName: whereHelpernull_String{field: "[dbo].[order_mark_codes].[service_provider_name]"},
	ServiceProviderRole: whereHelpernull_String{field: "[dbo].[order_mark_codes].[service_provider_role]"},
	Comment:             whereHelpernull_String{field: "[dbo].[order_mark_codes].[comment]"},
	Version:             whereHelpernull_String{field: "[dbo].[order_mark_codes].[version]"},
	State:               whereHelpernull_String{field: "[dbo].[order_mark_codes].[state]"},
	Status:              whereHelpernull_String{field: "[dbo].[order_mark_codes].[status]"},
	OrderID:             whereHelpernull_String{field: "[dbo].[order_mark_codes].[order_id]"},
	Archive:             whereHelpernull_Int{field: "[dbo].[order_mark_codes].[archive]"},
	JSON:                whereHelpernull_String{field: "[dbo].[order_mark_codes].[json]"},
}
...
var (
	orderMarkCodeAllColumns            = []string{"id", "create_date", "gtin", "quantity", "serial_number_type", "template_id", "cis_type", "contact_person", "release_method_type", "create_method_type", "payment_type", "production_order_id", "product_name", "product_capacity", "product_shelf_life", "product_template", "service_provider_id", "service_provider_name", "service_provider_role", "comment", "version", "state", "status", "order_id", "archive", "json"}
	orderMarkCodeColumnsWithoutDefault = []string{"create_date", "gtin", "quantity", "serial_number_type", "template_id", "cis_type", "contact_person", "release_method_type", "create_method_type", "payment_type", "production_order_id", "product_name", "product_capacity", "product_shelf_life", "product_template", "service_provider_id", "service_provider_name", "service_provider_role", "comment", "version", "state", "status", "order_id", "archive", "json"}
	orderMarkCodeColumnsWithDefault    = []string{"id"}
...
func OrderMarkCodes(mods ...qm.QueryMod) orderMarkCodeQuery {
	mods = append(mods, qm.From("[dbo].[order_mark_codes]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[order_mark_codes].*"})
	}
...
func FindOrderMarkCodeG(ctx context.Context, iD int, selectCols ...string) (*OrderMarkCode, error) {
...
func FindOrderMarkCode(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*OrderMarkCode, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[order_mark_codes] where [id]=$1", sel,
	)
...
func (o *OrderMarkCode) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[order_mark_codes] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[order_mark_codes] %sDEFAULT VALUES%s"
		}
...
func (o *OrderMarkCode) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[order_mark_codes] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, orderMarkCodePrimaryKeyColumns),
		)
...
func (o OrderMarkCodeSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[order_mark_codes] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, orderMarkCodePrimaryKeyColumns, len(o)))
...
func (o *OrderMarkCode) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *OrderMarkCode) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, orderMarkCodeGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(orderMarkCodePrimaryKeyColumns, v) && strmangle.ContainsAny(orderMarkCodeColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("znakboil: unable to upsert order_mark_codes, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(orderMarkCodeAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("znakboil: unable to upsert order_mark_codes, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[order_mark_codes]", orderMarkCodePrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(orderMarkCodePrimaryKeyColumns))
		copy(whitelist, orderMarkCodePrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(orderMarkCodeType, orderMarkCodeMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *OrderMarkCode) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[order_mark_codes] WHERE [id]=$1"
...
func (o OrderMarkCodeSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[order_mark_codes] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, orderMarkCodePrimaryKeyColumns, len(o))
...
func (o *OrderMarkCodeSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[order_mark_codes].* FROM [dbo].[order_mark_codes] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, orderMarkCodePrimaryKeyColumns, len(*o))
...
func OrderMarkCodeExistsG(ctx context.Context, iD int) (bool, error) {
	return OrderMarkCodeExists(ctx, boil.GetContextDB(), iD)
}

// OrderMarkCodeExists checks if the OrderMarkCode row exists.
func OrderMarkCodeExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[order_mark_codes] where [id]=$1) then 1 else 0 end"
