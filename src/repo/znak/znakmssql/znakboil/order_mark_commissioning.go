...
var OrderMarkCommissioningWhere = struct {
	...
	ID:                whereHelperint{field: "[dbo].[order_mark_commissioning].[id]"},
	CreateDate:        whereHelpernull_String{field: "[dbo].[order_mark_commissioning].[create_date]"},
	DocType:           whereHelpernull_String{field: "[dbo].[order_mark_commissioning].[doc_type]"},
	DocNumber:         whereHelpernull_String{field: "[dbo].[order_mark_commissioning].[doc_number]"},
	DocDate:           whereHelpernull_String{field: "[dbo].[order_mark_commissioning].[doc_date]"},
	ImportDate:        whereHelpernull_String{field: "[dbo].[order_mark_commissioning].[import_date]"},
	ExporterName:      whereHelpernull_String{field: "[dbo].[order_mark_commissioning].[exporter_name]"},
	ExporterCountry:   whereHelpernull_String{field: "[dbo].[order_mark_commissioning].[exporter_country]"},
	ShipperNumber:     whereHelpernull_String{field: "[dbo].[order_mark_commissioning].[shipper_number]"},
	DeclarationNumber: whereHelpernull_String{field: "[dbo].[order_mark_commissioning].[declaration_number]"},
	DeclarationDate:   whereHelpernull_String{field: "[dbo].[order_mark_commissioning].[declaration_date]"},
	ProductionType:    whereHelpernull_String{field: "[dbo].[order_mark_commissioning].[production_type]"},
	ProductionDate:    whereHelpernull_String{field: "[dbo].[order_mark_commissioning].[production_date]"},
	Quantity:          whereHelpernull_String{field: "[dbo].[order_mark_commissioning].[quantity]"},
	Version:           whereHelpernull_String{field: "[dbo].[order_mark_commissioning].[version]"},
	State:             whereHelpernull_String{field: "[dbo].[order_mark_commissioning].[state]"},
	Status:            whereHelpernull_String{field: "[dbo].[order_mark_commissioning].[status]"},
	OrderID:           whereHelpernull_String{field: "[dbo].[order_mark_commissioning].[order_id]"},
	Archive:           whereHelpernull_Int{field: "[dbo].[order_mark_commissioning].[archive]"},
	JSON:              whereHelpernull_String{field: "[dbo].[order_mark_commissioning].[json]"},
}
...
var (
	orderMarkCommissioningAllColumns            = []string{"id", "create_date", "doc_type", "doc_number", "doc_date", "import_date", "exporter_name", "exporter_country", "shipper_number", "declaration_number", "declaration_date", "production_type", "production_date", "quantity", "version", "state", "status", "order_id", "archive", "json"}
	orderMarkCommissioningColumnsWithoutDefault = []string{"create_date", "doc_type", "doc_number", "doc_date", "import_date", "exporter_name", "exporter_country", "shipper_number", "declaration_number", "declaration_date", "production_type", "production_date", "quantity", "version", "state", "status", "order_id", "archive", "json"}
	orderMarkCommissioningColumnsWithDefault    = []string{"id"}
...
func OrderMarkCommissionings(mods ...qm.QueryMod) orderMarkCommissioningQuery {
	mods = append(mods, qm.From("[dbo].[order_mark_commissioning]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[order_mark_commissioning].*"})
	}
...
func FindOrderMarkCommissioningG(ctx context.Context, iD int, selectCols ...string) (*OrderMarkCommissioning, error) {
...
func FindOrderMarkCommissioning(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*OrderMarkCommissioning, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[order_mark_commissioning] where [id]=$1", sel,
	)
...
func (o *OrderMarkCommissioning) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[order_mark_commissioning] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[order_mark_commissioning] %sDEFAULT VALUES%s"
		}
...
func (o *OrderMarkCommissioning) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[order_mark_commissioning] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, orderMarkCommissioningPrimaryKeyColumns),
		)
...
func (o OrderMarkCommissioningSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[order_mark_commissioning] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, orderMarkCommissioningPrimaryKeyColumns, len(o)))
...
func (o *OrderMarkCommissioning) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *OrderMarkCommissioning) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, orderMarkCommissioningGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(orderMarkCommissioningPrimaryKeyColumns, v) && strmangle.ContainsAny(orderMarkCommissioningColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("znakboil: unable to upsert order_mark_commissioning, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(orderMarkCommissioningAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("znakboil: unable to upsert order_mark_commissioning, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[order_mark_commissioning]", orderMarkCommissioningPrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(orderMarkCommissioningPrimaryKeyColumns))
		copy(whitelist, orderMarkCommissioningPrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(orderMarkCommissioningType, orderMarkCommissioningMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *OrderMarkCommissioning) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[order_mark_commissioning] WHERE [id]=$1"
...
func (o OrderMarkCommissioningSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[order_mark_commissioning] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, orderMarkCommissioningPrimaryKeyColumns, len(o))
...
func (o *OrderMarkCommissioningSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[order_mark_commissioning].* FROM [dbo].[order_mark_commissioning] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, orderMarkCommissioningPrimaryKeyColumns, len(*o))
...
func OrderMarkCommissioningExistsG(ctx context.Context, iD int) (bool, error) {
...
func OrderMarkCommissioningExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[order_mark_commissioning] where [id]=$1) then 1 else 0 end"
