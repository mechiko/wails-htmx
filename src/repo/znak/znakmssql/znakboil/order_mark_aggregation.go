...
var OrderMarkAggregationWhere = struct {
	...
	ID:                      whereHelperint{field: "[dbo].[order_mark_aggregation].[id]"},
	CreateDate:              whereHelpernull_String{field: "[dbo].[order_mark_aggregation].[create_date]"},
	Inn:                     whereHelpernull_String{field: "[dbo].[order_mark_aggregation].[inn]"},
	UnitSerialNumber:        whereHelpernull_String{field: "[dbo].[order_mark_aggregation].[unit_serial_number]"},
	AggregationUnitCapacity: whereHelpernull_String{field: "[dbo].[order_mark_aggregation].[aggregation_unit_capacity]"},
	AggregatedItemsCount:    whereHelpernull_String{field: "[dbo].[order_mark_aggregation].[aggregated_items_count]"},
	AggregationType:         whereHelpernull_String{field: "[dbo].[order_mark_aggregation].[aggregation_type]"},
	Version:                 whereHelpernull_String{field: "[dbo].[order_mark_aggregation].[version]"},
	State:                   whereHelpernull_String{field: "[dbo].[order_mark_aggregation].[state]"},
	Status:                  whereHelpernull_String{field: "[dbo].[order_mark_aggregation].[status]"},
	OrderID:                 whereHelpernull_String{field: "[dbo].[order_mark_aggregation].[order_id]"},
	Archive:                 whereHelpernull_Int{field: "[dbo].[order_mark_aggregation].[archive]"},
	JSON:                    whereHelpernull_String{field: "[dbo].[order_mark_aggregation].[json]"},
}
...
var (
	...
	orderMarkAggregationColumnsWithoutDefault = []string{"create_date", "inn", "unit_serial_number", "aggregation_unit_capacity", "aggregated_items_count", "aggregation_type", "version", "state", "status", "order_id", "archive", "json"}
	orderMarkAggregationColumnsWithDefault    = []string{"id"}
...
func OrderMarkAggregations(mods ...qm.QueryMod) orderMarkAggregationQuery {
	mods = append(mods, qm.From("[dbo].[order_mark_aggregation]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[order_mark_aggregation].*"})
	}
...
func FindOrderMarkAggregationG(ctx context.Context, iD int, selectCols ...string) (*OrderMarkAggregation, error) {
...
func FindOrderMarkAggregation(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*OrderMarkAggregation, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[order_mark_aggregation] where [id]=$1", sel,
	)
...
func (o *OrderMarkAggregation) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[order_mark_aggregation] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[order_mark_aggregation] %sDEFAULT VALUES%s"
		}
...
func (o *OrderMarkAggregation) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[order_mark_aggregation] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, orderMarkAggregationPrimaryKeyColumns),
		)
...
func (o OrderMarkAggregationSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[order_mark_aggregation] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, orderMarkAggregationPrimaryKeyColumns, len(o)))
...
func (o *OrderMarkAggregation) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *OrderMarkAggregation) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, orderMarkAggregationGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(orderMarkAggregationPrimaryKeyColumns, v) && strmangle.ContainsAny(orderMarkAggregationColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("znakboil: unable to upsert order_mark_aggregation, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(orderMarkAggregationAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("znakboil: unable to upsert order_mark_aggregation, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[order_mark_aggregation]", orderMarkAggregationPrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(orderMarkAggregationPrimaryKeyColumns))
		copy(whitelist, orderMarkAggregationPrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(orderMarkAggregationType, orderMarkAggregationMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *OrderMarkAggregation) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[order_mark_aggregation] WHERE [id]=$1"
...
func (o OrderMarkAggregationSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[order_mark_aggregation] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, orderMarkAggregationPrimaryKeyColumns, len(o))
...
func (o *OrderMarkAggregationSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[order_mark_aggregation].* FROM [dbo].[order_mark_aggregation] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, orderMarkAggregationPrimaryKeyColumns, len(*o))
...
func OrderMarkAggregationExistsG(ctx context.Context, iD int) (bool, error) {
...
func OrderMarkAggregationExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[order_mark_aggregation] where [id]=$1) then 1 else 0 end"
