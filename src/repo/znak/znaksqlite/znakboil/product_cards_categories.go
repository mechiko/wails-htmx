...
type ProductCardsCategory struct {
	...
	CatID          null.Int64  `boil:"cat_id" json:"cat_id,omitempty" toml:"cat_id" yaml:"cat_id,omitempty"`
...
var ProductCardsCategoryWhere = struct {
	...
	CatID          whereHelpernull_Int64
	CatName        whereHelpernull_String
}{
	ID:             whereHelperint64{field: "\"product_cards_categories\".\"id\""},
	IDProductCards: whereHelpernull_Int64{field: "\"product_cards_categories\".\"id_product_cards\""},
	CatID:          whereHelpernull_Int64{field: "\"product_cards_categories\".\"cat_id\""},
	CatName:        whereHelpernull_String{field: "\"product_cards_categories\".\"cat_name\""},
}
...
var (
	...
	productCardsCategoryColumnsWithoutDefault = []string{}
	productCardsCategoryColumnsWithDefault    = []string{"id", "id_product_cards", "cat_id", "cat_name"}
...
func ProductCardsCategories(mods ...qm.QueryMod) productCardsCategoryQuery {
	mods = append(mods, qm.From("\"product_cards_categories\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"product_cards_categories\".*"})
	}
...
func FindProductCardsCategoryG(ctx context.Context, iD int64, selectCols ...string) (*ProductCardsCategory, error) {
...
func FindProductCardsCategory(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*ProductCardsCategory, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"product_cards_categories\" where \"id\"=?", sel,
	)
...
func (o *ProductCardsCategory) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"product_cards_categories\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"product_cards_categories\" %sDEFAULT VALUES%s"
		}
...
func (o *ProductCardsCategory) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"product_cards_categories\" SET %s WHERE %s",
			strmangle.SetParamNames("\"", "\"", 0, wl),
			strmangle.WhereClause("\"", "\"", 0, productCardsCategoryPrimaryKeyColumns),
		)
...
func (o ProductCardsCategorySlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"product_cards_categories\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 0, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, productCardsCategoryPrimaryKeyColumns, len(o)))
...
func (o *ProductCardsCategory) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *ProductCardsCategory) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 {
			conflict = make([]string, len(productCardsCategoryPrimaryKeyColumns))
			copy(conflict, productCardsCategoryPrimaryKeyColumns)
		}
		cache.query = buildUpsertQuerySQLite(dialect, "\"product_cards_categories\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(productCardsCategoryType, productCardsCategoryMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *ProductCardsCategory) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"product_cards_categories\" WHERE \"id\"=?"
...
func (o ProductCardsCategorySlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"product_cards_categories\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, productCardsCategoryPrimaryKeyColumns, len(o))
...
func (o *ProductCardsCategorySlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"product_cards_categories\".* FROM \"product_cards_categories\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, productCardsCategoryPrimaryKeyColumns, len(*o))
...
func ProductCardsCategoryExistsG(ctx context.Context, iD int64) (bool, error) {
...
func ProductCardsCategoryExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"product_cards_categories\" where \"id\"=? limit 1)"
