...
type OrderMarkConnectingTapCode struct {
	...
	IDOrderMarkConnectingTap null.Int64  `boil:"id_order_mark_connecting_tap" json:"id_order_mark_connecting_tap,omitempty" toml:"id_order_mark_connecting_tap" yaml:"id_order_mark_connecting_tap,omitempty"`
...
var OrderMarkConnectingTapCodeWhere = struct {
	ID                       whereHelperint64
	IDOrderMarkConnectingTap whereHelpernull_Int64
	...
	ID:                       whereHelperint64{field: "\"order_mark_connecting_tap_codes\".\"id\""},
	IDOrderMarkConnectingTap: whereHelpernull_Int64{field: "\"order_mark_connecting_tap_codes\".\"id_order_mark_connecting_tap\""},
	SerialNumber:             whereHelpernull_String{field: "\"order_mark_connecting_tap_codes\".\"serial_number\""},
	Code:                     whereHelpernull_String{field: "\"order_mark_connecting_tap_codes\".\"code\""},
	ConnectDate:              whereHelpernull_String{field: "\"order_mark_connecting_tap_codes\".\"connect_date\""},
	ExpirationDate:           whereHelpernull_String{field: "\"order_mark_connecting_tap_codes\".\"expiration_date\""},
	Status:                   whereHelpernull_String{field: "\"order_mark_connecting_tap_codes\".\"status\""},
}
...
var (
	...
	orderMarkConnectingTapCodeColumnsWithoutDefault = []string{}
	orderMarkConnectingTapCodeColumnsWithDefault    = []string{"id", "id_order_mark_connecting_tap", "serial_number", "code", "connect_date", "expiration_date", "status"}
...
func OrderMarkConnectingTapCodes(mods ...qm.QueryMod) orderMarkConnectingTapCodeQuery {
	mods = append(mods, qm.From("\"order_mark_connecting_tap_codes\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"order_mark_connecting_tap_codes\".*"})
	}
...
func FindOrderMarkConnectingTapCodeG(ctx context.Context, iD int64, selectCols ...string) (*OrderMarkConnectingTapCode, error) {
...
func FindOrderMarkConnectingTapCode(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*OrderMarkConnectingTapCode, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"order_mark_connecting_tap_codes\" where \"id\"=?", sel,
	)
...
func (o *OrderMarkConnectingTapCode) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"order_mark_connecting_tap_codes\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"order_mark_connecting_tap_codes\" %sDEFAULT VALUES%s"
		}
...
func (o *OrderMarkConnectingTapCode) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"order_mark_connecting_tap_codes\" SET %s WHERE %s",
			strmangle.SetParamNames("\"", "\"", 0, wl),
			strmangle.WhereClause("\"", "\"", 0, orderMarkConnectingTapCodePrimaryKeyColumns),
		)
...
func (o OrderMarkConnectingTapCodeSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"order_mark_connecting_tap_codes\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 0, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, orderMarkConnectingTapCodePrimaryKeyColumns, len(o)))
...
func (o *OrderMarkConnectingTapCode) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *OrderMarkConnectingTapCode) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 {
			conflict = make([]string, len(orderMarkConnectingTapCodePrimaryKeyColumns))
			copy(conflict, orderMarkConnectingTapCodePrimaryKeyColumns)
		}
		cache.query = buildUpsertQuerySQLite(dialect, "\"order_mark_connecting_tap_codes\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(orderMarkConnectingTapCodeType, orderMarkConnectingTapCodeMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *OrderMarkConnectingTapCode) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"order_mark_connecting_tap_codes\" WHERE \"id\"=?"
...
func (o OrderMarkConnectingTapCodeSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"order_mark_connecting_tap_codes\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, orderMarkConnectingTapCodePrimaryKeyColumns, len(o))
...
func (o *OrderMarkConnectingTapCodeSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"order_mark_connecting_tap_codes\".* FROM \"order_mark_connecting_tap_codes\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, orderMarkConnectingTapCodePrimaryKeyColumns, len(*o))
...
func OrderMarkConnectingTapCodeExistsG(ctx context.Context, iD int64) (bool, error) {
...
func OrderMarkConnectingTapCodeExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"order_mark_connecting_tap_codes\" where \"id\"=? limit 1)"
