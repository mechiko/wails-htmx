...
const defaultSrc = `header\templates`
...
func (t *page) Render(w io.Writer, templateName string, data interface{}) error {
	if templateName == "" {
		templateName = defaultTemplateName
	}
	if domain.Mode == "development" {
		return t.doRender(w, templateName, data)
	} else {
		if t.template == nil {
...
func (t *page) doRender(w io.Writer, templateName string, data interface{}) (err error) {
	defer func() {
		// ловим панику тут
		if r := recover(); r != nil {
	...
	template.Must(tt.New("content").Funcs(funcMapHtml).Parse(utility.FilePackageContents("content.html")))
