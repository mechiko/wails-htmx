...
const defaultSrc = `dbinfo\templates`
...
func (t *page) DoRender(w io.Writer, templateName string, data interface{}, c echo.Context) (err error) {
	...
	template.Must(tt.New("style").Parse(utility.FilePackageContents("style.html")))
	template.Must(tt.New("index").Funcs(funcMapHtml).Parse(utility.FilePackageContents("index.html")))
	template.Must(tt.New("indexerror").Funcs(funcMapHtml).Parse(utility.FilePackageContents("indexerror.html")))
	...
	template.Must(tt.New("error").Funcs(funcMapHtml).Parse(utility.FilePackageContents("error.html")))
