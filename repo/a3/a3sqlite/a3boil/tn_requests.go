...
type TNRequest struct {
	...
	IDTN         null.Int64  `boil:"id_tn" json:"id_tn,omitempty" toml:"id_tn" yaml:"id_tn,omitempty"`
...
var TNRequestWhere = struct {
	ID           whereHelperint64
	IDTN         whereHelpernull_Int64
	RequestDate  whereHelpernull_String
	RequestRegID whereHelpernull_String
	Status       whereHelpernull_String
	ReplyID      whereHelpernull_String
}{
	ID:           whereHelperint64{field: "\"tn_requests\".\"id\""},
	IDTN:         whereHelpernull_Int64{field: "\"tn_requests\".\"id_tn\""},
	RequestDate:  whereHelpernull_String{field: "\"tn_requests\".\"request_date\""},
	RequestRegID: whereHelpernull_String{field: "\"tn_requests\".\"request_reg_id\""},
	Status:       whereHelpernull_String{field: "\"tn_requests\".\"status\""},
	ReplyID:      whereHelpernull_String{field: "\"tn_requests\".\"reply_id\""},
}
...
var (
	...
	tnRequestColumnsWithoutDefault = []string{}
	tnRequestColumnsWithDefault    = []string{"id", "id_tn", "request_date", "request_reg_id", "status", "reply_id"}
...
func TNRequests(mods ...qm.QueryMod) tnRequestQuery {
	mods = append(mods, qm.From("\"tn_requests\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"tn_requests\".*"})
	}
...
func FindTNRequestG(ctx context.Context, iD int64, selectCols ...string) (*TNRequest, error) {
...
func FindTNRequest(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*TNRequest, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"tn_requests\" where \"id\"=?", sel,
	)
...
func (o *TNRequest) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"tn_requests\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"tn_requests\" %sDEFAULT VALUES%s"
		}
...
func (o *TNRequest) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"tn_requests\" SET %s WHERE %s",
			strmangle.SetParamNames("\"", "\"", 0, wl),
			strmangle.WhereClause("\"", "\"", 0, tnRequestPrimaryKeyColumns),
		)
...
func (o TNRequestSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"tn_requests\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 0, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, tnRequestPrimaryKeyColumns, len(o)))
...
func (o *TNRequest) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *TNRequest) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 {
			conflict = make([]string, len(tnRequestPrimaryKeyColumns))
			copy(conflict, tnRequestPrimaryKeyColumns)
		}
		cache.query = buildUpsertQuerySQLite(dialect, "\"tn_requests\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(tnRequestType, tnRequestMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *TNRequest) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"tn_requests\" WHERE \"id\"=?"
...
func (o TNRequestSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"tn_requests\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, tnRequestPrimaryKeyColumns, len(o))
...
func (o *TNRequestSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"tn_requests\".* FROM \"tn_requests\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, tnRequestPrimaryKeyColumns, len(*o))
...
func TNRequestExistsG(ctx context.Context, iD int64) (bool, error) {
	return TNRequestExists(ctx, boil.GetContextDB(), iD)
}

// TNRequestExists checks if the TNRequest row exists.
func TNRequestExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"tn_requests\" where \"id\"=? limit 1)"
