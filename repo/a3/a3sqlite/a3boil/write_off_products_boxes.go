...
type WriteOffProductsBox struct {
	...
	IDWriteOffProductsPallets null.Int64  `boil:"id_write_off_products_pallets" json:"id_write_off_products_pallets,omitempty" toml:"id_write_off_products_pallets" yaml:"id_write_off_products_pallets,omitempty"`
...
var WriteOffProductsBoxWhere = struct {
	...
	IDWriteOffProductsPallets whereHelpernull_Int64
	BoxNumber                 whereHelpernull_String
}{
	ID:                        whereHelperint64{field: "\"write_off_products_boxes\".\"id\""},
	IDWriteOffProducts:        whereHelpernull_Int64{field: "\"write_off_products_boxes\".\"id_write_off_products\""},
	IDWriteOffProductsPallets: whereHelpernull_Int64{field: "\"write_off_products_boxes\".\"id_write_off_products_pallets\""},
	BoxNumber:                 whereHelpernull_String{field: "\"write_off_products_boxes\".\"box_number\""},
}
...
var (
	...
	writeOffProductsBoxColumnsWithoutDefault = []string{}
	writeOffProductsBoxColumnsWithDefault    = []string{"id", "id_write_off_products", "id_write_off_products_pallets", "box_number"}
...
func WriteOffProductsBoxes(mods ...qm.QueryMod) writeOffProductsBoxQuery {
	mods = append(mods, qm.From("\"write_off_products_boxes\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"write_off_products_boxes\".*"})
	}
...
func FindWriteOffProductsBoxG(ctx context.Context, iD int64, selectCols ...string) (*WriteOffProductsBox, error) {
...
func FindWriteOffProductsBox(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*WriteOffProductsBox, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"write_off_products_boxes\" where \"id\"=?", sel,
	)
...
func (o *WriteOffProductsBox) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"write_off_products_boxes\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"write_off_products_boxes\" %sDEFAULT VALUES%s"
		}
...
func (o *WriteOffProductsBox) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"write_off_products_boxes\" SET %s WHERE %s",
			strmangle.SetParamNames("\"", "\"", 0, wl),
			strmangle.WhereClause("\"", "\"", 0, writeOffProductsBoxPrimaryKeyColumns),
		)
...
func (o WriteOffProductsBoxSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"write_off_products_boxes\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 0, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, writeOffProductsBoxPrimaryKeyColumns, len(o)))
...
func (o *WriteOffProductsBox) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *WriteOffProductsBox) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 {
			conflict = make([]string, len(writeOffProductsBoxPrimaryKeyColumns))
			copy(conflict, writeOffProductsBoxPrimaryKeyColumns)
		}
		cache.query = buildUpsertQuerySQLite(dialect, "\"write_off_products_boxes\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(writeOffProductsBoxType, writeOffProductsBoxMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *WriteOffProductsBox) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"write_off_products_boxes\" WHERE \"id\"=?"
...
func (o WriteOffProductsBoxSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"write_off_products_boxes\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, writeOffProductsBoxPrimaryKeyColumns, len(o))
...
func (o *WriteOffProductsBoxSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"write_off_products_boxes\".* FROM \"write_off_products_boxes\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, writeOffProductsBoxPrimaryKeyColumns, len(*o))
...
func WriteOffProductsBoxExistsG(ctx context.Context, iD int64) (bool, error) {
...
func WriteOffProductsBoxExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"write_off_products_boxes\" where \"id\"=? limit 1)"
