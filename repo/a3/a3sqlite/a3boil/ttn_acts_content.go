...
var TTNActsContentWhere = struct {
	...
	ID:                   whereHelperint64{field: "\"ttn_acts_content\".\"id\""},
	IDTTNActs:            whereHelpernull_Int64{field: "\"ttn_acts_content\".\"id_ttn_acts\""},
	ProductIdentity:      whereHelpernull_String{field: "\"ttn_acts_content\".\"product_identity\""},
	ProductInformF2RegID: whereHelpernull_String{field: "\"ttn_acts_content\".\"product_inform_f2_reg_id\""},
	ProductQuantity:      whereHelpernull_String{field: "\"ttn_acts_content\".\"product_quantity\""},
}
...
var (
	...
	ttnActsContentColumnsWithoutDefault = []string{}
	ttnActsContentColumnsWithDefault    = []string{"id", "id_ttn_acts", "product_identity", "product_inform_f2_reg_id", "product_quantity"}
...
func TTNActsContents(mods ...qm.QueryMod) ttnActsContentQuery {
	mods = append(mods, qm.From("\"ttn_acts_content\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"ttn_acts_content\".*"})
	}
...
func FindTTNActsContentG(ctx context.Context, iD int64, selectCols ...string) (*TTNActsContent, error) {
...
func FindTTNActsContent(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*TTNActsContent, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"ttn_acts_content\" where \"id\"=?", sel,
	)
...
func (o *TTNActsContent) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"ttn_acts_content\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"ttn_acts_content\" %sDEFAULT VALUES%s"
		}
...
func (o *TTNActsContent) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"ttn_acts_content\" SET %s WHERE %s",
			strmangle.SetParamNames("\"", "\"", 0, wl),
			strmangle.WhereClause("\"", "\"", 0, ttnActsContentPrimaryKeyColumns),
		)
...
func (o TTNActsContentSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"ttn_acts_content\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 0, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, ttnActsContentPrimaryKeyColumns, len(o)))
...
func (o *TTNActsContent) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *TTNActsContent) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 {
			conflict = make([]string, len(ttnActsContentPrimaryKeyColumns))
			copy(conflict, ttnActsContentPrimaryKeyColumns)
		}
		cache.query = buildUpsertQuerySQLite(dialect, "\"ttn_acts_content\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(ttnActsContentType, ttnActsContentMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *TTNActsContent) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"ttn_acts_content\" WHERE \"id\"=?"
...
func (o TTNActsContentSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"ttn_acts_content\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, ttnActsContentPrimaryKeyColumns, len(o))
...
func (o *TTNActsContentSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"ttn_acts_content\".* FROM \"ttn_acts_content\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, ttnActsContentPrimaryKeyColumns, len(*o))
...
func TTNActsContentExistsG(ctx context.Context, iD int64) (bool, error) {
	return TTNActsContentExists(ctx, boil.GetContextDB(), iD)
}

// TTNActsContentExists checks if the TTNActsContent row exists.
func TTNActsContentExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"ttn_acts_content\" where \"id\"=? limit 1)"
