...
var TNRouteWhere = struct {
	...
	ID:    whereHelperint64{field: "\"tn_routes\".\"id\""},
	IDTN:  whereHelpernull_Int64{field: "\"tn_routes\".\"id_tn\""},
	Route: whereHelpernull_String{field: "\"tn_routes\".\"route\""},
}
...
var (
	...
	tnRouteColumnsWithoutDefault = []string{}
	tnRouteColumnsWithDefault    = []string{"id", "id_tn", "route"}
...
func TNRoutes(mods ...qm.QueryMod) tnRouteQuery {
	mods = append(mods, qm.From("\"tn_routes\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"tn_routes\".*"})
	}
...
func FindTNRouteG(ctx context.Context, iD int64, selectCols ...string) (*TNRoute, error) {
...
func FindTNRoute(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*TNRoute, error) {
	tnRouteObj := &TNRoute{}
	...
	query := fmt.Sprintf(
		"select %s from \"tn_routes\" where \"id\"=?", sel,
	)
...
func (o *TNRoute) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"tn_routes\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"tn_routes\" %sDEFAULT VALUES%s"
		}
...
func (o *TNRoute) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"tn_routes\" SET %s WHERE %s",
			strmangle.SetParamNames("\"", "\"", 0, wl),
			strmangle.WhereClause("\"", "\"", 0, tnRoutePrimaryKeyColumns),
		)
...
func (o TNRouteSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"tn_routes\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 0, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, tnRoutePrimaryKeyColumns, len(o)))
...
func (o *TNRoute) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *TNRoute) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 {
			conflict = make([]string, len(tnRoutePrimaryKeyColumns))
			copy(conflict, tnRoutePrimaryKeyColumns)
		}
		cache.query = buildUpsertQuerySQLite(dialect, "\"tn_routes\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(tnRouteType, tnRouteMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *TNRoute) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"tn_routes\" WHERE \"id\"=?"
...
func (o TNRouteSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"tn_routes\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, tnRoutePrimaryKeyColumns, len(o))
...
func (o *TNRouteSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"tn_routes\".* FROM \"tn_routes\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, tnRoutePrimaryKeyColumns, len(*o))
...
func TNRouteExistsG(ctx context.Context, iD int64) (bool, error) {
	return TNRouteExists(ctx, boil.GetContextDB(), iD)
}

// TNRouteExists checks if the TNRoute row exists.
func TNRouteExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"tn_routes\" where \"id\"=? limit 1)"
