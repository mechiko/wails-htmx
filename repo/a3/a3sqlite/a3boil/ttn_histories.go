...
var TTNHistoryWhere = struct {
	...
	ID:       whereHelperint64{field: "\"ttn_histories\".\"id\""},
	IDTTN:    whereHelpernull_Int64{field: "\"ttn_histories\".\"id_ttn\""},
	Identity: whereHelpernull_String{field: "\"ttn_histories\".\"identity\""},
}
...
var (
	...
	ttnHistoryColumnsWithoutDefault = []string{}
	ttnHistoryColumnsWithDefault    = []string{"id", "id_ttn", "identity"}
...
func TTNHistories(mods ...qm.QueryMod) ttnHistoryQuery {
	mods = append(mods, qm.From("\"ttn_histories\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"ttn_histories\".*"})
	}
...
func FindTTNHistoryG(ctx context.Context, iD int64, selectCols ...string) (*TTNHistory, error) {
...
func FindTTNHistory(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*TTNHistory, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"ttn_histories\" where \"id\"=?", sel,
	)
...
func (o *TTNHistory) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"ttn_histories\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"ttn_histories\" %sDEFAULT VALUES%s"
		}
...
func (o *TTNHistory) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"ttn_histories\" SET %s WHERE %s",
			strmangle.SetParamNames("\"", "\"", 0, wl),
			strmangle.WhereClause("\"", "\"", 0, ttnHistoryPrimaryKeyColumns),
		)
...
func (o TTNHistorySlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"ttn_histories\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 0, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, ttnHistoryPrimaryKeyColumns, len(o)))
...
func (o *TTNHistory) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *TTNHistory) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 {
			conflict = make([]string, len(ttnHistoryPrimaryKeyColumns))
			copy(conflict, ttnHistoryPrimaryKeyColumns)
		}
		cache.query = buildUpsertQuerySQLite(dialect, "\"ttn_histories\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(ttnHistoryType, ttnHistoryMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *TTNHistory) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"ttn_histories\" WHERE \"id\"=?"
...
func (o TTNHistorySlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"ttn_histories\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, ttnHistoryPrimaryKeyColumns, len(o))
...
func (o *TTNHistorySlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"ttn_histories\".* FROM \"ttn_histories\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, ttnHistoryPrimaryKeyColumns, len(*o))
...
func TTNHistoryExistsG(ctx context.Context, iD int64) (bool, error) {
	return TTNHistoryExists(ctx, boil.GetContextDB(), iD)
}

// TTNHistoryExists checks if the TTNHistory row exists.
func TTNHistoryExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"ttn_histories\" where \"id\"=? limit 1)"
