...
var ChargeOnActWhere = struct {
	...
	ID:               whereHelperint64{field: "\"charge_on_acts\".\"id\""},
	CreateDate:       whereHelpernull_String{field: "\"charge_on_acts\".\"create_date\""},
	DocIdentity:      whereHelpernull_String{field: "\"charge_on_acts\".\"doc_identity\""},
	DocType:          whereHelpernull_String{field: "\"charge_on_acts\".\"doc_type\""},
	DocNumber:        whereHelpernull_String{field: "\"charge_on_acts\".\"doc_number\""},
	DocDate:          whereHelpernull_String{field: "\"charge_on_acts\".\"doc_date\""},
	DocComment:       whereHelpernull_String{field: "\"charge_on_acts\".\"doc_comment\""},
	WriteOffActRegID: whereHelpernull_String{field: "\"charge_on_acts\".\"write_off_act_reg_id\""},
	Version:          whereHelpernull_String{field: "\"charge_on_acts\".\"version\""},
	State:            whereHelpernull_String{field: "\"charge_on_acts\".\"state\""},
	Status:           whereHelpernull_String{field: "\"charge_on_acts\".\"status\""},
	ReplyID:          whereHelpernull_String{field: "\"charge_on_acts\".\"reply_id\""},
	Archive:          whereHelpernull_Int64{field: "\"charge_on_acts\".\"archive\""},
	XML:              whereHelpernull_String{field: "\"charge_on_acts\".\"xml\""},
}
...
var (
	...
	chargeOnActColumnsWithoutDefault = []string{}
	chargeOnActColumnsWithDefault    = []string{"id", "create_date", "doc_identity", "doc_type", "doc_number", "doc_date", "doc_comment", "write_off_act_reg_id", "version", "state", "status", "reply_id", "archive", "xml"}
...
func ChargeOnActs(mods ...qm.QueryMod) chargeOnActQuery {
	mods = append(mods, qm.From("\"charge_on_acts\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"charge_on_acts\".*"})
	}
...
func FindChargeOnActG(ctx context.Context, iD int64, selectCols ...string) (*ChargeOnAct, error) {
...
func FindChargeOnAct(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*ChargeOnAct, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"charge_on_acts\" where \"id\"=?", sel,
	)
...
func (o *ChargeOnAct) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"charge_on_acts\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"charge_on_acts\" %sDEFAULT VALUES%s"
		}
...
func (o *ChargeOnAct) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"charge_on_acts\" SET %s WHERE %s",
			strmangle.SetParamNames("\"", "\"", 0, wl),
			strmangle.WhereClause("\"", "\"", 0, chargeOnActPrimaryKeyColumns),
		)
...
func (o ChargeOnActSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"charge_on_acts\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 0, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, chargeOnActPrimaryKeyColumns, len(o)))
...
func (o *ChargeOnAct) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *ChargeOnAct) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 {
			conflict = make([]string, len(chargeOnActPrimaryKeyColumns))
			copy(conflict, chargeOnActPrimaryKeyColumns)
		}
		cache.query = buildUpsertQuerySQLite(dialect, "\"charge_on_acts\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(chargeOnActType, chargeOnActMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *ChargeOnAct) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"charge_on_acts\" WHERE \"id\"=?"
...
func (o ChargeOnActSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"charge_on_acts\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, chargeOnActPrimaryKeyColumns, len(o))
...
func (o *ChargeOnActSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"charge_on_acts\".* FROM \"charge_on_acts\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, chargeOnActPrimaryKeyColumns, len(*o))
...
func ChargeOnActExistsG(ctx context.Context, iD int64) (bool, error) {
	return ChargeOnActExists(ctx, boil.GetContextDB(), iD)
}

// ChargeOnActExists checks if the ChargeOnAct row exists.
func ChargeOnActExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"charge_on_acts\" where \"id\"=? limit 1)"
