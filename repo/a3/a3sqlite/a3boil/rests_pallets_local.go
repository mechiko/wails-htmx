...
var RestsPalletsLocalWhere = struct {
	...
	ID:            whereHelperint64{field: "\"rests_pallets_local\".\"id\""},
	PalletNumber:  whereHelpernull_String{field: "\"rests_pallets_local\".\"pallet_number\""},
	InformF2RegID: whereHelpernull_String{field: "\"rests_pallets_local\".\"inform_f2_reg_id\""},
	Archive:       whereHelpernull_Int64{field: "\"rests_pallets_local\".\"archive\""},
}
...
var (
	...
	restsPalletsLocalColumnsWithoutDefault = []string{}
	restsPalletsLocalColumnsWithDefault    = []string{"id", "pallet_number", "inform_f2_reg_id", "archive"}
...
func RestsPalletsLocals(mods ...qm.QueryMod) restsPalletsLocalQuery {
	mods = append(mods, qm.From("\"rests_pallets_local\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"rests_pallets_local\".*"})
	}
...
func FindRestsPalletsLocalG(ctx context.Context, iD int64, selectCols ...string) (*RestsPalletsLocal, error) {
...
func FindRestsPalletsLocal(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*RestsPalletsLocal, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"rests_pallets_local\" where \"id\"=?", sel,
	)
...
func (o *RestsPalletsLocal) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"rests_pallets_local\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"rests_pallets_local\" %sDEFAULT VALUES%s"
		}
...
func (o *RestsPalletsLocal) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"rests_pallets_local\" SET %s WHERE %s",
			strmangle.SetParamNames("\"", "\"", 0, wl),
			strmangle.WhereClause("\"", "\"", 0, restsPalletsLocalPrimaryKeyColumns),
		)
...
func (o RestsPalletsLocalSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"rests_pallets_local\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 0, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, restsPalletsLocalPrimaryKeyColumns, len(o)))
...
func (o *RestsPalletsLocal) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *RestsPalletsLocal) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 {
			conflict = make([]string, len(restsPalletsLocalPrimaryKeyColumns))
			copy(conflict, restsPalletsLocalPrimaryKeyColumns)
		}
		cache.query = buildUpsertQuerySQLite(dialect, "\"rests_pallets_local\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(restsPalletsLocalType, restsPalletsLocalMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *RestsPalletsLocal) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"rests_pallets_local\" WHERE \"id\"=?"
...
func (o RestsPalletsLocalSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"rests_pallets_local\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, restsPalletsLocalPrimaryKeyColumns, len(o))
...
func (o *RestsPalletsLocalSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"rests_pallets_local\".* FROM \"rests_pallets_local\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, restsPalletsLocalPrimaryKeyColumns, len(*o))
...
func RestsPalletsLocalExistsG(ctx context.Context, iD int64) (bool, error) {
...
func RestsPalletsLocalExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"rests_pallets_local\" where \"id\"=? limit 1)"
