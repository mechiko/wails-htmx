...
var ProductionQueryWhere = struct {
	...
	ID:                  whereHelperint64{field: "\"production_queries\".\"id\""},
	IDProductionReports: whereHelpernull_Int64{field: "\"production_queries\".\"id_production_reports\""},
	QueryDate:           whereHelpernull_String{field: "\"production_queries\".\"query_date\""},
	QueryRegID:          whereHelpernull_String{field: "\"production_queries\".\"query_reg_id\""},
	Status:              whereHelpernull_String{field: "\"production_queries\".\"status\""},
	ReplyID:             whereHelpernull_String{field: "\"production_queries\".\"reply_id\""},
}
...
var (
	...
	productionQueryColumnsWithoutDefault = []string{}
	productionQueryColumnsWithDefault    = []string{"id", "id_production_reports", "query_date", "query_reg_id", "status", "reply_id"}
...
func ProductionQueries(mods ...qm.QueryMod) productionQueryQuery {
	mods = append(mods, qm.From("\"production_queries\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"production_queries\".*"})
	}
...
func FindProductionQueryG(ctx context.Context, iD int64, selectCols ...string) (*ProductionQuery, error) {
...
func FindProductionQuery(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*ProductionQuery, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"production_queries\" where \"id\"=?", sel,
	)
...
func (o *ProductionQuery) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"production_queries\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"production_queries\" %sDEFAULT VALUES%s"
		}
...
func (o *ProductionQuery) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"production_queries\" SET %s WHERE %s",
			strmangle.SetParamNames("\"", "\"", 0, wl),
			strmangle.WhereClause("\"", "\"", 0, productionQueryPrimaryKeyColumns),
		)
...
func (o ProductionQuerySlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"production_queries\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 0, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, productionQueryPrimaryKeyColumns, len(o)))
...
func (o *ProductionQuery) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *ProductionQuery) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 {
			conflict = make([]string, len(productionQueryPrimaryKeyColumns))
			copy(conflict, productionQueryPrimaryKeyColumns)
		}
		cache.query = buildUpsertQuerySQLite(dialect, "\"production_queries\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(productionQueryType, productionQueryMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *ProductionQuery) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"production_queries\" WHERE \"id\"=?"
...
func (o ProductionQuerySlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"production_queries\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, productionQueryPrimaryKeyColumns, len(o))
...
func (o *ProductionQuerySlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"production_queries\".* FROM \"production_queries\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, productionQueryPrimaryKeyColumns, len(*o))
...
func ProductionQueryExistsG(ctx context.Context, iD int64) (bool, error) {
	return ProductionQueryExists(ctx, boil.GetContextDB(), iD)
}

// ProductionQueryExists checks if the ProductionQuery row exists.
func ProductionQueryExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"production_queries\" where \"id\"=? limit 1)"
