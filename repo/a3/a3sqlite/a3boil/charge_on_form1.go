...
var ChargeOnForm1Where = struct {
	...
	ID:             whereHelperint64{field: "\"charge_on_form1\".\"id\""},
	IDChargeOnActs: whereHelpernull_Int64{field: "\"charge_on_form1\".\"id_charge_on_acts\""},
	DocIddomain:    whereHelpernull_String{field: "\"charge_on_form1\".\"doc_iddomain\""},
	DocRegID:       whereHelpernull_String{field: "\"charge_on_form1\".\"doc_reg_id\""},
	DocNumber:      whereHelpernull_String{field: "\"charge_on_form1\".\"doc_number\""},
	XML:            whereHelpernull_String{field: "\"charge_on_form1\".\"xml\""},
}
...
var (
	...
	chargeOnForm1ColumnsWithoutDefault = []string{}
	chargeOnForm1ColumnsWithDefault    = []string{"id", "id_charge_on_acts", "doc_iddomain", "doc_reg_id", "doc_number", "xml"}
...
func ChargeOnForm1s(mods ...qm.QueryMod) chargeOnForm1Query {
	mods = append(mods, qm.From("\"charge_on_form1\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"charge_on_form1\".*"})
	}
...
func FindChargeOnForm1G(ctx context.Context, iD int64, selectCols ...string) (*ChargeOnForm1, error) {
...
func FindChargeOnForm1(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*ChargeOnForm1, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"charge_on_form1\" where \"id\"=?", sel,
	)
...
func (o *ChargeOnForm1) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"charge_on_form1\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"charge_on_form1\" %sDEFAULT VALUES%s"
		}
...
func (o *ChargeOnForm1) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"charge_on_form1\" SET %s WHERE %s",
			strmangle.SetParamNames("\"", "\"", 0, wl),
			strmangle.WhereClause("\"", "\"", 0, chargeOnForm1PrimaryKeyColumns),
		)
...
func (o ChargeOnForm1Slice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"charge_on_form1\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 0, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, chargeOnForm1PrimaryKeyColumns, len(o)))
...
func (o *ChargeOnForm1) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *ChargeOnForm1) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 {
			conflict = make([]string, len(chargeOnForm1PrimaryKeyColumns))
			copy(conflict, chargeOnForm1PrimaryKeyColumns)
		}
		cache.query = buildUpsertQuerySQLite(dialect, "\"charge_on_form1\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(chargeOnForm1Type, chargeOnForm1Mapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *ChargeOnForm1) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"charge_on_form1\" WHERE \"id\"=?"
...
func (o ChargeOnForm1Slice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"charge_on_form1\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, chargeOnForm1PrimaryKeyColumns, len(o))
...
func (o *ChargeOnForm1Slice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"charge_on_form1\".* FROM \"charge_on_form1\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, chargeOnForm1PrimaryKeyColumns, len(*o))
...
func ChargeOnForm1ExistsG(ctx context.Context, iD int64) (bool, error) {
	return ChargeOnForm1Exists(ctx, boil.GetContextDB(), iD)
}

// ChargeOnForm1Exists checks if the ChargeOnForm1 row exists.
func ChargeOnForm1Exists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"charge_on_form1\" where \"id\"=? limit 1)"
