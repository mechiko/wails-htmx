...
var ProductionGrapeWhere = struct {
	...
	ID:                   whereHelperint64{field: "\"production_grapes\".\"id\""},
	IDProductionReports:  whereHelpernull_Int64{field: "\"production_grapes\".\"id_production_reports\""},
	IDProductionProducts: whereHelpernull_Int64{field: "\"production_grapes\".\"id_production_products\""},
	GrapeSort:            whereHelpernull_String{field: "\"production_grapes\".\"grape_sort\""},
	GrapeCode:            whereHelpernull_String{field: "\"production_grapes\".\"grape_code\""},
	...
	SupplierKPP:          whereHelpernull_String{field: "\"production_grapes\".\"supplier_kpp\""},
	SupplierFullName:     whereHelpernull_String{field: "\"production_grapes\".\"supplier_full_name\""},
	SupplierShortName:    whereHelpernull_String{field: "\"production_grapes\".\"supplier_short_name\""},
	SupplierCountryCode:  whereHelpernull_String{field: "\"production_grapes\".\"supplier_country_code\""},
	SupplierRegionCode:   whereHelpernull_String{field: "\"production_grapes\".\"supplier_region_code\""},
	SupplierDescription:  whereHelpernull_String{field: "\"production_grapes\".\"supplier_description\""},
}
...
func ProductionGrapes(mods ...qm.QueryMod) productionGrapeQuery {
	mods = append(mods, qm.From("\"production_grapes\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"production_grapes\".*"})
	}
...
func FindProductionGrapeG(ctx context.Context, iD int64, selectCols ...string) (*ProductionGrape, error) {
...
func FindProductionGrape(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*ProductionGrape, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"production_grapes\" where \"id\"=?", sel,
	)
...
func (o *ProductionGrape) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"production_grapes\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"production_grapes\" %sDEFAULT VALUES%s"
		}
...
func (o *ProductionGrape) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"production_grapes\" SET %s WHERE %s",
			strmangle.SetParamNames("\"", "\"", 0, wl),
			strmangle.WhereClause("\"", "\"", 0, productionGrapePrimaryKeyColumns),
		)
...
func (o ProductionGrapeSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"production_grapes\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 0, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, productionGrapePrimaryKeyColumns, len(o)))
...
func (o *ProductionGrape) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *ProductionGrape) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 {
			conflict = make([]string, len(productionGrapePrimaryKeyColumns))
			copy(conflict, productionGrapePrimaryKeyColumns)
		}
		cache.query = buildUpsertQuerySQLite(dialect, "\"production_grapes\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(productionGrapeType, productionGrapeMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *ProductionGrape) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"production_grapes\" WHERE \"id\"=?"
...
func (o ProductionGrapeSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"production_grapes\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, productionGrapePrimaryKeyColumns, len(o))
...
func (o *ProductionGrapeSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"production_grapes\".* FROM \"production_grapes\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, productionGrapePrimaryKeyColumns, len(*o))
...
func ProductionGrapeExistsG(ctx context.Context, iD int64) (bool, error) {
	return ProductionGrapeExists(ctx, boil.GetContextDB(), iD)
}

// ProductionGrapeExists checks if the ProductionGrape row exists.
func ProductionGrapeExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"production_grapes\" where \"id\"=? limit 1)"
