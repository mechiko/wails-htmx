...
var SpEgaisWhere = struct {
	...
	ID:               whereHelperint64{field: "\"sp_egais\".\"id\""},
	IDRequests:       whereHelpernull_Int64{field: "\"sp_egais\".\"id_requests\""},
	ProductFullName:  whereHelpernull_String{field: "\"sp_egais\".\"product_full_name\""},
	ProductCapacity:  whereHelpernull_String{field: "\"sp_egais\".\"product_capacity\""},
	ProductAlcVolume: whereHelpernull_String{field: "\"sp_egais\".\"product_alc_volume\""},
	ProductAlcCode:   whereHelpernull_String{field: "\"sp_egais\".\"product_alc_code\""},
	ProductCode:      whereHelpernull_String{field: "\"sp_egais\".\"product_code\""},
	ProductUnitType:  whereHelpernull_String{field: "\"sp_egais\".\"product_unit_type\""},
}
...
var (
	...
	spEgaisColumnsWithoutDefault = []string{}
	spEgaisColumnsWithDefault    = []string{"id", "id_requests", "product_full_name", "product_capacity", "product_alc_volume", "product_alc_code", "product_code", "product_unit_type"}
...
func SpEgaiss(mods ...qm.QueryMod) spEgaisQuery {
	mods = append(mods, qm.From("\"sp_egais\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"sp_egais\".*"})
	}
...
func FindSpEgaisG(ctx context.Context, iD int64, selectCols ...string) (*SpEgais, error) {
...
func FindSpEgais(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*SpEgais, error) {
	spEgaisObj := &SpEgais{}
	...
	query := fmt.Sprintf(
		"select %s from \"sp_egais\" where \"id\"=?", sel,
	)
...
func (o *SpEgais) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"sp_egais\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"sp_egais\" %sDEFAULT VALUES%s"
		}
...
func (o *SpEgais) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"sp_egais\" SET %s WHERE %s",
			strmangle.SetParamNames("\"", "\"", 0, wl),
			strmangle.WhereClause("\"", "\"", 0, spEgaisPrimaryKeyColumns),
		)
...
func (o SpEgaisSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"sp_egais\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 0, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, spEgaisPrimaryKeyColumns, len(o)))
...
func (o *SpEgais) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *SpEgais) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 {
			conflict = make([]string, len(spEgaisPrimaryKeyColumns))
			copy(conflict, spEgaisPrimaryKeyColumns)
		}
		cache.query = buildUpsertQuerySQLite(dialect, "\"sp_egais\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(spEgaisType, spEgaisMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *SpEgais) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"sp_egais\" WHERE \"id\"=?"
...
func (o SpEgaisSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"sp_egais\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, spEgaisPrimaryKeyColumns, len(o))
...
func (o *SpEgaisSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"sp_egais\".* FROM \"sp_egais\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, spEgaisPrimaryKeyColumns, len(*o))
...
func SpEgaisExistsG(ctx context.Context, iD int64) (bool, error) {
	return SpEgaisExists(ctx, boil.GetContextDB(), iD)
}

// SpEgaisExists checks if the SpEgais row exists.
func SpEgaisExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"sp_egais\" where \"id\"=? limit 1)"
