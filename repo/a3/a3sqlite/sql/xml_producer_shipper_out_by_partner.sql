...
select DISTINCT ROW_NUMBER() OVER (ORDER BY (SELECT 1)) AS 'row1',
  ...
  cast(COALESCE(fa.producer_full_name , tp.producer_full_name, '') as varchar) as full_name,
  ...
  cast(COALESCE(fa.producer_description , tp.producer_description, '') as varchar) as description
FROM
...
select DISTINCT ROW_NUMBER() OVER (ORDER BY (SELECT 1)) AS 'row2',
  ...
  cast(COALESCE(fb.shipper_full_name, tt.shipper_full_name) as varchar) as full_name,
  ...
  cast(COALESCE(fb.shipper_description, tt.shipper_description) as varchar) as description
FROM
  ttn tt join ttn_products tp on tt.id = tp.id_ttn
...
SELECT group_concat(title,',') as 'title', reg_id,org_type, inn, kpp, full_name, short_name, country_code, region_code, description
FROM (
