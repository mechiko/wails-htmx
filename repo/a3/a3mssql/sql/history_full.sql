...
select
  COALESCE((SELECT TOP 1 doc_fix_date FROM ttn_form2 WHERE id_ttn = tt.id), tt.doc_date) as 'fix_date',
  'ТТН' as 'type',
  ...
  COALESCE((SELECT TOP 1 doc_reg_id FROM ttn_form2 WHERE id_ttn = tt.id), '') as 'doc_reg_id',
...
select
  ...
  '' as 'partner_fsrar_id',
  tp.producer_client_reg_id as 'producer_fsrar_id',
  coalesce((SELECT TOP 1 CAST(product_alc_volume AS float) FROM form1_egais WHERE product_inform_f1_reg_id = tp.resource_inform_f1_reg_id ), 0.0) as 'alc_volume_fact',
  coalesce((SELECT TOP 1 product_inform_f1_reg_id FROM form1_egais WHERE product_inform_f1_reg_id = tp.resource_inform_f1_reg_id ), concat('отсутствует ', tp.resource_inform_f1_reg_id)) as 'form1',
  coalesce((SELECT TOP 1 product_inform_f2_reg_id FROM form2_egais WHERE product_inform_f2_reg_id = tp.resource_inform_f2_reg_id ), concat('отсутствует ', tp.resource_inform_f2_reg_id)) as 'form2',
  '' as 'fix_number',
  COALESCE(tf.doc_reg_id,'отсутствует') as 'doc_reg_id',
  coalesce((SELECT TOP 1 bottling_date FROM form1_egais WHERE product_inform_f1_reg_id = tp.resource_inform_f1_reg_id ), concat('отсутствует ', tp.resource_inform_f1_reg_id)) as 'bottling_date',
