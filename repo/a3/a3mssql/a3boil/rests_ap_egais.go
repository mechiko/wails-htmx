...
var RestsApEgaisWhere = struct {
	...
	ID:                   whereHelperint{field: "[dbo].[rests_ap_egais].[id]"},
	IDRequests:           whereHelpernull_Int{field: "[dbo].[rests_ap_egais].[id_requests]"},
	ProductFullName:      whereHelpernull_String{field: "[dbo].[rests_ap_egais].[product_full_name]"},
	ProductCapacity:      whereHelpernull_String{field: "[dbo].[rests_ap_egais].[product_capacity]"},
	ProductAlcVolume:     whereHelpernull_String{field: "[dbo].[rests_ap_egais].[product_alc_volume]"},
	ProductAlcCode:       whereHelpernull_String{field: "[dbo].[rests_ap_egais].[product_alc_code]"},
	ProductCode:          whereHelpernull_String{field: "[dbo].[rests_ap_egais].[product_code]"},
	ProductUnitType:      whereHelpernull_String{field: "[dbo].[rests_ap_egais].[product_unit_type]"},
	ProductQuantity:      whereHelpernull_String{field: "[dbo].[rests_ap_egais].[product_quantity]"},
	ProductInformF1RegID: whereHelpernull_String{field: "[dbo].[rests_ap_egais].[product_inform_f1_reg_id]"},
	ProductInformF2RegID: whereHelpernull_String{field: "[dbo].[rests_ap_egais].[product_inform_f2_reg_id]"},
	ProducerType:         whereHelpernull_String{field: "[dbo].[rests_ap_egais].[producer_type]"},
	ProducerClientRegID:  whereHelpernull_String{field: "[dbo].[rests_ap_egais].[producer_client_reg_id]"},
	ProducerInn:          whereHelpernull_String{field: "[dbo].[rests_ap_egais].[producer_inn]"},
	ProducerKPP:          whereHelpernull_String{field: "[dbo].[rests_ap_egais].[producer_kpp]"},
	ProducerFullName:     whereHelpernull_String{field: "[dbo].[rests_ap_egais].[producer_full_name]"},
	ProducerShortName:    whereHelpernull_String{field: "[dbo].[rests_ap_egais].[producer_short_name]"},
	ProducerCountryCode:  whereHelpernull_String{field: "[dbo].[rests_ap_egais].[producer_country_code]"},
	ProducerRegionCode:   whereHelpernull_String{field: "[dbo].[rests_ap_egais].[producer_region_code]"},
	ProducerDescription:  whereHelpernull_String{field: "[dbo].[rests_ap_egais].[producer_description]"},
	RestsApDate:          whereHelpernull_String{field: "[dbo].[rests_ap_egais].[rests_ap_date]"},
}
...
var (
	...
	restsApEgaisColumnsWithoutDefault = []string{"id_requests", "product_full_name", "product_capacity", "product_alc_volume", "product_alc_code", "product_code", "product_unit_type", "product_quantity", "product_inform_f1_reg_id", "product_inform_f2_reg_id", "producer_type", "producer_client_reg_id", "producer_inn", "producer_kpp", "producer_full_name", "producer_short_name", "producer_country_code", "producer_region_code", "producer_description", "rests_ap_date"}
	restsApEgaisColumnsWithDefault    = []string{"id"}
...
func RestsApEgaiss(mods ...qm.QueryMod) restsApEgaisQuery {
	mods = append(mods, qm.From("[dbo].[rests_ap_egais]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[rests_ap_egais].*"})
	}
...
func FindRestsApEgaisG(ctx context.Context, iD int, selectCols ...string) (*RestsApEgais, error) {
...
func FindRestsApEgais(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*RestsApEgais, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[rests_ap_egais] where [id]=$1", sel,
	)
...
func (o *RestsApEgais) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[rests_ap_egais] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[rests_ap_egais] %sDEFAULT VALUES%s"
		}
...
func (o *RestsApEgais) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[rests_ap_egais] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, restsApEgaisPrimaryKeyColumns),
		)
...
func (o RestsApEgaisSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[rests_ap_egais] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, restsApEgaisPrimaryKeyColumns, len(o)))
...
func (o *RestsApEgais) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *RestsApEgais) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, restsApEgaisGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(restsApEgaisPrimaryKeyColumns, v) && strmangle.ContainsAny(restsApEgaisColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("a3boil: unable to upsert rests_ap_egais, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(restsApEgaisAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("a3boil: unable to upsert rests_ap_egais, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[rests_ap_egais]", restsApEgaisPrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(restsApEgaisPrimaryKeyColumns))
		copy(whitelist, restsApEgaisPrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(restsApEgaisType, restsApEgaisMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *RestsApEgais) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[rests_ap_egais] WHERE [id]=$1"
...
func (o RestsApEgaisSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[rests_ap_egais] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, restsApEgaisPrimaryKeyColumns, len(o))
...
func (o *RestsApEgaisSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[rests_ap_egais].* FROM [dbo].[rests_ap_egais] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, restsApEgaisPrimaryKeyColumns, len(*o))
...
func RestsApEgaisExistsG(ctx context.Context, iD int) (bool, error) {
	return RestsApEgaisExists(ctx, boil.GetContextDB(), iD)
}

// RestsApEgaisExists checks if the RestsApEgais row exists.
func RestsApEgaisExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[rests_ap_egais] where [id]=$1) then 1 else 0 end"
