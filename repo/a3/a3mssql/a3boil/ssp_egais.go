...
var SspEgaisWhere = struct {
	...
	ID:                  whereHelperint{field: "[dbo].[ssp_egais].[id]"},
	IDRequests:          whereHelpernull_Int{field: "[dbo].[ssp_egais].[id_requests]"},
	ProductFullName:     whereHelpernull_String{field: "[dbo].[ssp_egais].[product_full_name]"},
	ProductCapacity:     whereHelpernull_String{field: "[dbo].[ssp_egais].[product_capacity]"},
	ProductAlcVolume:    whereHelpernull_String{field: "[dbo].[ssp_egais].[product_alc_volume]"},
	ProductAlcCode:      whereHelpernull_String{field: "[dbo].[ssp_egais].[product_alc_code]"},
	ProductCode:         whereHelpernull_String{field: "[dbo].[ssp_egais].[product_code]"},
	ProductUnitType:     whereHelpernull_String{field: "[dbo].[ssp_egais].[product_unit_type]"},
	ProducerType:        whereHelpernull_String{field: "[dbo].[ssp_egais].[producer_type]"},
	ProducerClientRegID: whereHelpernull_String{field: "[dbo].[ssp_egais].[producer_client_reg_id]"},
	ProducerInn:         whereHelpernull_String{field: "[dbo].[ssp_egais].[producer_inn]"},
	ProducerKPP:         whereHelpernull_String{field: "[dbo].[ssp_egais].[producer_kpp]"},
	ProducerFullName:    whereHelpernull_String{field: "[dbo].[ssp_egais].[producer_full_name]"},
	ProducerShortName:   whereHelpernull_String{field: "[dbo].[ssp_egais].[producer_short_name]"},
	ProducerCountryCode: whereHelpernull_String{field: "[dbo].[ssp_egais].[producer_country_code]"},
	ProducerRegionCode:  whereHelpernull_String{field: "[dbo].[ssp_egais].[producer_region_code]"},
	ProducerDescription: whereHelpernull_String{field: "[dbo].[ssp_egais].[producer_description]"},
}
...
var (
	...
	sspEgaisColumnsWithoutDefault = []string{"id_requests", "product_full_name", "product_capacity", "product_alc_volume", "product_alc_code", "product_code", "product_unit_type", "producer_type", "producer_client_reg_id", "producer_inn", "producer_kpp", "producer_full_name", "producer_short_name", "producer_country_code", "producer_region_code", "producer_description"}
	sspEgaisColumnsWithDefault    = []string{"id"}
...
func SspEgaiss(mods ...qm.QueryMod) sspEgaisQuery {
	mods = append(mods, qm.From("[dbo].[ssp_egais]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[ssp_egais].*"})
	}
...
func FindSspEgaisG(ctx context.Context, iD int, selectCols ...string) (*SspEgais, error) {
...
func FindSspEgais(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*SspEgais, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[ssp_egais] where [id]=$1", sel,
	)
...
func (o *SspEgais) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[ssp_egais] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[ssp_egais] %sDEFAULT VALUES%s"
		}
...
func (o *SspEgais) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[ssp_egais] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, sspEgaisPrimaryKeyColumns),
		)
...
func (o SspEgaisSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[ssp_egais] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, sspEgaisPrimaryKeyColumns, len(o)))
...
func (o *SspEgais) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *SspEgais) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, sspEgaisGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(sspEgaisPrimaryKeyColumns, v) && strmangle.ContainsAny(sspEgaisColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("a3boil: unable to upsert ssp_egais, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(sspEgaisAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("a3boil: unable to upsert ssp_egais, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[ssp_egais]", sspEgaisPrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(sspEgaisPrimaryKeyColumns))
		copy(whitelist, sspEgaisPrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(sspEgaisType, sspEgaisMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *SspEgais) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[ssp_egais] WHERE [id]=$1"
...
func (o SspEgaisSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[ssp_egais] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, sspEgaisPrimaryKeyColumns, len(o))
...
func (o *SspEgaisSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[ssp_egais].* FROM [dbo].[ssp_egais] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, sspEgaisPrimaryKeyColumns, len(*o))
...
func SspEgaisExistsG(ctx context.Context, iD int) (bool, error) {
	return SspEgaisExists(ctx, boil.GetContextDB(), iD)
}

// SspEgaisExists checks if the SspEgais row exists.
func SspEgaisExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[ssp_egais] where [id]=$1) then 1 else 0 end"
