...
var ProductionForm1Where = struct {
	...
	ID:                  whereHelperint{field: "[dbo].[production_form1].[id]"},
	IDProductionReports: whereHelpernull_Int{field: "[dbo].[production_form1].[id_production_reports]"},
	DocIdentity:         whereHelpernull_String{field: "[dbo].[production_form1].[doc_identity]"},
	DocRegID:            whereHelpernull_String{field: "[dbo].[production_form1].[doc_reg_id]"},
	ClientType:          whereHelpernull_String{field: "[dbo].[production_form1].[client_type]"},
	ClientRegID:         whereHelpernull_String{field: "[dbo].[production_form1].[client_reg_id]"},
	ClientInn:           whereHelpernull_String{field: "[dbo].[production_form1].[client_inn]"},
	ClientKPP:           whereHelpernull_String{field: "[dbo].[production_form1].[client_kpp]"},
	ClientFullName:      whereHelpernull_String{field: "[dbo].[production_form1].[client_full_name]"},
	ClientShortName:     whereHelpernull_String{field: "[dbo].[production_form1].[client_short_name]"},
	ClientCountryCode:   whereHelpernull_String{field: "[dbo].[production_form1].[client_country_code]"},
	ClientRegionCode:    whereHelpernull_String{field: "[dbo].[production_form1].[client_region_code]"},
	ClientDescription:   whereHelpernull_String{field: "[dbo].[production_form1].[client_description]"},
	XML:                 whereHelpernull_String{field: "[dbo].[production_form1].[xml]"},
}
...
var (
	...
	productionForm1ColumnsWithoutDefault = []string{"id_production_reports", "doc_identity", "doc_reg_id", "client_type", "client_reg_id", "client_inn", "client_kpp", "client_full_name", "client_short_name", "client_country_code", "client_region_code", "client_description", "xml"}
	productionForm1ColumnsWithDefault    = []string{"id"}
...
func ProductionForm1s(mods ...qm.QueryMod) productionForm1Query {
	mods = append(mods, qm.From("[dbo].[production_form1]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[production_form1].*"})
	}
...
func FindProductionForm1G(ctx context.Context, iD int, selectCols ...string) (*ProductionForm1, error) {
...
func FindProductionForm1(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*ProductionForm1, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[production_form1] where [id]=$1", sel,
	)
...
func (o *ProductionForm1) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[production_form1] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[production_form1] %sDEFAULT VALUES%s"
		}
...
func (o *ProductionForm1) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[production_form1] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, productionForm1PrimaryKeyColumns),
		)
...
func (o ProductionForm1Slice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[production_form1] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, productionForm1PrimaryKeyColumns, len(o)))
...
func (o *ProductionForm1) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *ProductionForm1) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, productionForm1GeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(productionForm1PrimaryKeyColumns, v) && strmangle.ContainsAny(productionForm1ColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("a3boil: unable to upsert production_form1, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(productionForm1AllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("a3boil: unable to upsert production_form1, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[production_form1]", productionForm1PrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(productionForm1PrimaryKeyColumns))
		copy(whitelist, productionForm1PrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(productionForm1Type, productionForm1Mapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *ProductionForm1) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[production_form1] WHERE [id]=$1"
...
func (o ProductionForm1Slice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[production_form1] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, productionForm1PrimaryKeyColumns, len(o))
...
func (o *ProductionForm1Slice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[production_form1].* FROM [dbo].[production_form1] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, productionForm1PrimaryKeyColumns, len(*o))
...
func ProductionForm1ExistsG(ctx context.Context, iD int) (bool, error) {
	return ProductionForm1Exists(ctx, boil.GetContextDB(), iD)
}

// ProductionForm1Exists checks if the ProductionForm1 row exists.
func ProductionForm1Exists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[production_form1] where [id]=$1) then 1 else 0 end"
