...
var SSPLocalWhere = struct {
	...
	ID:                  whereHelperint{field: "[dbo].[ssp_local].[id]"},
	ProductFullName:     whereHelpernull_String{field: "[dbo].[ssp_local].[product_full_name]"},
	ProductCapacity:     whereHelpernull_String{field: "[dbo].[ssp_local].[product_capacity]"},
	ProductAlcVolume:    whereHelpernull_String{field: "[dbo].[ssp_local].[product_alc_volume]"},
	ProductAlcCode:      whereHelpernull_String{field: "[dbo].[ssp_local].[product_alc_code]"},
	ProductCode:         whereHelpernull_String{field: "[dbo].[ssp_local].[product_code]"},
	ProductUnitType:     whereHelpernull_String{field: "[dbo].[ssp_local].[product_unit_type]"},
	ProducerType:        whereHelpernull_String{field: "[dbo].[ssp_local].[producer_type]"},
	ProducerClientRegID: whereHelpernull_String{field: "[dbo].[ssp_local].[producer_client_reg_id]"},
	ProducerInn:         whereHelpernull_String{field: "[dbo].[ssp_local].[producer_inn]"},
	ProducerKPP:         whereHelpernull_String{field: "[dbo].[ssp_local].[producer_kpp]"},
	ProducerFullName:    whereHelpernull_String{field: "[dbo].[ssp_local].[producer_full_name]"},
	ProducerShortName:   whereHelpernull_String{field: "[dbo].[ssp_local].[producer_short_name]"},
	ProducerCountryCode: whereHelpernull_String{field: "[dbo].[ssp_local].[producer_country_code]"},
	ProducerRegionCode:  whereHelpernull_String{field: "[dbo].[ssp_local].[producer_region_code]"},
	ProducerDescription: whereHelpernull_String{field: "[dbo].[ssp_local].[producer_description]"},
}
...
var (
	...
	sspLocalColumnsWithoutDefault = []string{"product_full_name", "product_capacity", "product_alc_volume", "product_alc_code", "product_code", "product_unit_type", "producer_type", "producer_client_reg_id", "producer_inn", "producer_kpp", "producer_full_name", "producer_short_name", "producer_country_code", "producer_region_code", "producer_description"}
	sspLocalColumnsWithDefault    = []string{"id"}
...
func SSPLocals(mods ...qm.QueryMod) sspLocalQuery {
	mods = append(mods, qm.From("[dbo].[ssp_local]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[ssp_local].*"})
	}
...
func FindSSPLocalG(ctx context.Context, iD int, selectCols ...string) (*SSPLocal, error) {
...
func FindSSPLocal(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*SSPLocal, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[ssp_local] where [id]=$1", sel,
	)
...
func (o *SSPLocal) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[ssp_local] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[ssp_local] %sDEFAULT VALUES%s"
		}
...
func (o *SSPLocal) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[ssp_local] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, sspLocalPrimaryKeyColumns),
		)
...
func (o SSPLocalSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[ssp_local] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, sspLocalPrimaryKeyColumns, len(o)))
...
func (o *SSPLocal) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *SSPLocal) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, sspLocalGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(sspLocalPrimaryKeyColumns, v) && strmangle.ContainsAny(sspLocalColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("a3boil: unable to upsert ssp_local, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(sspLocalAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("a3boil: unable to upsert ssp_local, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[ssp_local]", sspLocalPrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(sspLocalPrimaryKeyColumns))
		copy(whitelist, sspLocalPrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(sspLocalType, sspLocalMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *SSPLocal) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[ssp_local] WHERE [id]=$1"
...
func (o SSPLocalSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[ssp_local] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, sspLocalPrimaryKeyColumns, len(o))
...
func (o *SSPLocalSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[ssp_local].* FROM [dbo].[ssp_local] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, sspLocalPrimaryKeyColumns, len(*o))
...
func SSPLocalExistsG(ctx context.Context, iD int) (bool, error) {
	return SSPLocalExists(ctx, boil.GetContextDB(), iD)
}

// SSPLocalExists checks if the SSPLocal row exists.
func SSPLocalExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[ssp_local] where [id]=$1) then 1 else 0 end"
