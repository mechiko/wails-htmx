...
func ProductionForm1Contents(mods ...qm.QueryMod) productionForm1ContentQuery {
	mods = append(mods, qm.From("[dbo].[production_form1_content]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[production_form1_content].*"})
	}
...
func FindProductionForm1ContentG(ctx context.Context, iD int, selectCols ...string) (*ProductionForm1Content, error) {
...
func FindProductionForm1Content(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*ProductionForm1Content, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[production_form1_content] where [id]=$1", sel,
	)
...
func (o *ProductionForm1Content) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[production_form1_content] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[production_form1_content] %sDEFAULT VALUES%s"
		}
...
func (o *ProductionForm1Content) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[production_form1_content] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, productionForm1ContentPrimaryKeyColumns),
		)
...
func (o ProductionForm1ContentSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[production_form1_content] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, productionForm1ContentPrimaryKeyColumns, len(o)))
...
func (o *ProductionForm1Content) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *ProductionForm1Content) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, productionForm1ContentGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(productionForm1ContentPrimaryKeyColumns, v) && strmangle.ContainsAny(productionForm1ContentColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("a3boil: unable to upsert production_form1_content, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(productionForm1ContentAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("a3boil: unable to upsert production_form1_content, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[production_form1_content]", productionForm1ContentPrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(productionForm1ContentPrimaryKeyColumns))
		copy(whitelist, productionForm1ContentPrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(productionForm1ContentType, productionForm1ContentMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *ProductionForm1Content) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[production_form1_content] WHERE [id]=$1"
...
func (o ProductionForm1ContentSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[production_form1_content] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, productionForm1ContentPrimaryKeyColumns, len(o))
...
func (o *ProductionForm1ContentSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[production_form1_content].* FROM [dbo].[production_form1_content] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, productionForm1ContentPrimaryKeyColumns, len(*o))
...
func ProductionForm1ContentExistsG(ctx context.Context, iD int) (bool, error) {
...
func ProductionForm1ContentExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[production_form1_content] where [id]=$1) then 1 else 0 end"
