...
type WriteOffProduct struct {
	...
	IDWriteOffActs       null.Int    `boil:"id_write_off_acts" json:"id_write_off_acts,omitempty" toml:"id_write_off_acts" yaml:"id_write_off_acts,omitempty"`
...
var WriteOffProductWhere = struct {
	ID                   whereHelperint
	IDWriteOffActs       whereHelpernull_Int
	...
	ID:                   whereHelperint{field: "[dbo].[write_off_products].[id]"},
	IDWriteOffActs:       whereHelpernull_Int{field: "[dbo].[write_off_products].[id_write_off_acts]"},
	ProductFullName:      whereHelpernull_String{field: "[dbo].[write_off_products].[product_full_name]"},
	ProductCapacity:      whereHelpernull_String{field: "[dbo].[write_off_products].[product_capacity]"},
	ProductAlcVolume:     whereHelpernull_String{field: "[dbo].[write_off_products].[product_alc_volume]"},
	ProductAlcCode:       whereHelpernull_String{field: "[dbo].[write_off_products].[product_alc_code]"},
	ProductCode:          whereHelpernull_String{field: "[dbo].[write_off_products].[product_code]"},
	ProductUnitType:      whereHelpernull_String{field: "[dbo].[write_off_products].[product_unit_type]"},
	ProductIdentity:      whereHelpernull_String{field: "[dbo].[write_off_products].[product_identity]"},
	ProductQuantity:      whereHelpernull_String{field: "[dbo].[write_off_products].[product_quantity]"},
	ProductPrice:         whereHelpernull_String{field: "[dbo].[write_off_products].[product_price]"},
	ProductInformF1RegID: whereHelpernull_String{field: "[dbo].[write_off_products].[product_inform_f1_reg_id]"},
	ProductInformF2RegID: whereHelpernull_String{field: "[dbo].[write_off_products].[product_inform_f2_reg_id]"},
	ProducerType:         whereHelpernull_String{field: "[dbo].[write_off_products].[producer_type]"},
	ProducerClientRegID:  whereHelpernull_String{field: "[dbo].[write_off_products].[producer_client_reg_id]"},
	ProducerInn:          whereHelpernull_String{field: "[dbo].[write_off_products].[producer_inn]"},
	ProducerKPP:          whereHelpernull_String{field: "[dbo].[write_off_products].[producer_kpp]"},
	ProducerFullName:     whereHelpernull_String{field: "[dbo].[write_off_products].[producer_full_name]"},
	ProducerShortName:    whereHelpernull_String{field: "[dbo].[write_off_products].[producer_short_name]"},
	ProducerCountryCode:  whereHelpernull_String{field: "[dbo].[write_off_products].[producer_country_code]"},
	ProducerRegionCode:   whereHelpernull_String{field: "[dbo].[write_off_products].[producer_region_code]"},
	ProducerDescription:  whereHelpernull_String{field: "[dbo].[write_off_products].[producer_description]"},
}
...
var (
	...
	writeOffProductColumnsWithoutDefault = []string{"id_write_off_acts", "product_full_name", "product_capacity", "product_alc_volume", "product_alc_code", "product_code", "product_unit_type", "product_identity", "product_quantity", "product_price", "product_inform_f1_reg_id", "product_inform_f2_reg_id", "producer_type", "producer_client_reg_id", "producer_inn", "producer_kpp", "producer_full_name", "producer_short_name", "producer_country_code", "producer_region_code", "producer_description"}
	writeOffProductColumnsWithDefault    = []string{"id"}
...
func WriteOffProducts(mods ...qm.QueryMod) writeOffProductQuery {
	mods = append(mods, qm.From("[dbo].[write_off_products]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[write_off_products].*"})
	}
...
func FindWriteOffProductG(ctx context.Context, iD int, selectCols ...string) (*WriteOffProduct, error) {
...
func FindWriteOffProduct(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*WriteOffProduct, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[write_off_products] where [id]=$1", sel,
	)
...
func (o *WriteOffProduct) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[write_off_products] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[write_off_products] %sDEFAULT VALUES%s"
		}
...
func (o *WriteOffProduct) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[write_off_products] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, writeOffProductPrimaryKeyColumns),
		)
...
func (o WriteOffProductSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[write_off_products] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, writeOffProductPrimaryKeyColumns, len(o)))
...
func (o *WriteOffProduct) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *WriteOffProduct) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, writeOffProductGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(writeOffProductPrimaryKeyColumns, v) && strmangle.ContainsAny(writeOffProductColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("a3boil: unable to upsert write_off_products, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(writeOffProductAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("a3boil: unable to upsert write_off_products, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[write_off_products]", writeOffProductPrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(writeOffProductPrimaryKeyColumns))
		copy(whitelist, writeOffProductPrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(writeOffProductType, writeOffProductMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *WriteOffProduct) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[write_off_products] WHERE [id]=$1"
...
func (o WriteOffProductSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[write_off_products] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, writeOffProductPrimaryKeyColumns, len(o))
...
func (o *WriteOffProductSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[write_off_products].* FROM [dbo].[write_off_products] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, writeOffProductPrimaryKeyColumns, len(*o))
...
func WriteOffProductExistsG(ctx context.Context, iD int) (bool, error) {
	return WriteOffProductExists(ctx, boil.GetContextDB(), iD)
}

// WriteOffProductExists checks if the WriteOffProduct row exists.
func WriteOffProductExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[write_off_products] where [id]=$1) then 1 else 0 end"
