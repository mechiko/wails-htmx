...
func TTNForm2Contents(mods ...qm.QueryMod) ttnForm2ContentQuery {
	mods = append(mods, qm.From("[dbo].[ttn_form2_content]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[ttn_form2_content].*"})
	}
...
func FindTTNForm2ContentG(ctx context.Context, iD int, selectCols ...string) (*TTNForm2Content, error) {
...
func FindTTNForm2Content(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*TTNForm2Content, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[ttn_form2_content] where [id]=$1", sel,
	)
...
func (o *TTNForm2Content) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[ttn_form2_content] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[ttn_form2_content] %sDEFAULT VALUES%s"
		}
...
func (o *TTNForm2Content) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[ttn_form2_content] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, ttnForm2ContentPrimaryKeyColumns),
		)
...
func (o TTNForm2ContentSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[ttn_form2_content] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, ttnForm2ContentPrimaryKeyColumns, len(o)))
...
func (o *TTNForm2Content) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *TTNForm2Content) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, ttnForm2ContentGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(ttnForm2ContentPrimaryKeyColumns, v) && strmangle.ContainsAny(ttnForm2ContentColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("a3boil: unable to upsert ttn_form2_content, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(ttnForm2ContentAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("a3boil: unable to upsert ttn_form2_content, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[ttn_form2_content]", ttnForm2ContentPrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(ttnForm2ContentPrimaryKeyColumns))
		copy(whitelist, ttnForm2ContentPrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(ttnForm2ContentType, ttnForm2ContentMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *TTNForm2Content) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[ttn_form2_content] WHERE [id]=$1"
...
func (o TTNForm2ContentSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[ttn_form2_content] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, ttnForm2ContentPrimaryKeyColumns, len(o))
...
func (o *TTNForm2ContentSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[ttn_form2_content].* FROM [dbo].[ttn_form2_content] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, ttnForm2ContentPrimaryKeyColumns, len(*o))
...
func TTNForm2ContentExistsG(ctx context.Context, iD int) (bool, error) {
	return TTNForm2ContentExists(ctx, boil.GetContextDB(), iD)
}

// TTNForm2ContentExists checks if the TTNForm2Content row exists.
func TTNForm2ContentExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[ttn_form2_content] where [id]=$1) then 1 else 0 end"
