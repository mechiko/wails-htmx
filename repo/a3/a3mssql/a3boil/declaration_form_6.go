...
var DeclarationForm6Where = struct {
	...
	ID:                   whereHelperint{field: "[dbo].[declaration_form_6].[id]"},
	CreateDate:           whereHelpernull_String{field: "[dbo].[declaration_form_6].[create_date]"},
	Type:                 whereHelpernull_String{field: "[dbo].[declaration_form_6].[type]"},
	ReportingQuarter:     whereHelpernull_String{field: "[dbo].[declaration_form_6].[reporting_quarter]"},
	ReportingYear:        whereHelpernull_String{field: "[dbo].[declaration_form_6].[reporting_year]"},
	CorrectionNumber:     whereHelpernull_String{field: "[dbo].[declaration_form_6].[correction_number]"},
	DirectorSurname:      whereHelpernull_String{field: "[dbo].[declaration_form_6].[director_surname]"},
	DirectorName:         whereHelpernull_String{field: "[dbo].[declaration_form_6].[director_name]"},
	DirectorMiddleName:   whereHelpernull_String{field: "[dbo].[declaration_form_6].[director_middle_name]"},
	AccountantSurname:    whereHelpernull_String{field: "[dbo].[declaration_form_6].[accountant_surname]"},
	AccountantName:       whereHelpernull_String{field: "[dbo].[declaration_form_6].[accountant_name]"},
	AccountantMiddleName: whereHelpernull_String{field: "[dbo].[declaration_form_6].[accountant_middle_name]"},
	GUID:                 whereHelpernull_String{field: "[dbo].[declaration_form_6].[guid]"},
}
...
var (
	...
	declarationForm6ColumnsWithoutDefault = []string{"create_date", "type", "reporting_quarter", "reporting_year", "correction_number", "director_surname", "director_name", "director_middle_name", "accountant_surname", "accountant_name", "accountant_middle_name", "guid"}
	declarationForm6ColumnsWithDefault    = []string{"id"}
...
func DeclarationForm6S(mods ...qm.QueryMod) declarationForm6Query {
	mods = append(mods, qm.From("[dbo].[declaration_form_6]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[declaration_form_6].*"})
	}
...
func FindDeclarationForm6G(ctx context.Context, iD int, selectCols ...string) (*DeclarationForm6, error) {
...
func FindDeclarationForm6(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*DeclarationForm6, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[declaration_form_6] where [id]=$1", sel,
	)
...
func (o *DeclarationForm6) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[declaration_form_6] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[declaration_form_6] %sDEFAULT VALUES%s"
		}
...
func (o *DeclarationForm6) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[declaration_form_6] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, declarationForm6PrimaryKeyColumns),
		)
...
func (o DeclarationForm6Slice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[declaration_form_6] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, declarationForm6PrimaryKeyColumns, len(o)))
...
func (o *DeclarationForm6) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *DeclarationForm6) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, declarationForm6GeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(declarationForm6PrimaryKeyColumns, v) && strmangle.ContainsAny(declarationForm6ColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("a3boil: unable to upsert declaration_form_6, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(declarationForm6AllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("a3boil: unable to upsert declaration_form_6, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[declaration_form_6]", declarationForm6PrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(declarationForm6PrimaryKeyColumns))
		copy(whitelist, declarationForm6PrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(declarationForm6Type, declarationForm6Mapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *DeclarationForm6) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[declaration_form_6] WHERE [id]=$1"
...
func (o DeclarationForm6Slice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[declaration_form_6] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, declarationForm6PrimaryKeyColumns, len(o))
...
func (o *DeclarationForm6Slice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[declaration_form_6].* FROM [dbo].[declaration_form_6] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, declarationForm6PrimaryKeyColumns, len(*o))
...
func DeclarationForm6ExistsG(ctx context.Context, iD int) (bool, error) {
	return DeclarationForm6Exists(ctx, boil.GetContextDB(), iD)
}

// DeclarationForm6Exists checks if the DeclarationForm6 row exists.
func DeclarationForm6Exists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[declaration_form_6] where [id]=$1) then 1 else 0 end"
