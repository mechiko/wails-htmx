...
type ImportProductsMark struct {
	...
	IDImportProductsBoxes null.Int    `boil:"id_import_products_boxes" json:"id_import_products_boxes,omitempty" toml:"id_import_products_boxes" yaml:"id_import_products_boxes,omitempty"`
...
var ImportProductsMarkWhere = struct {
	...
	IDImportProductsBoxes whereHelpernull_Int
	Mark                  whereHelpernull_String
}{
	ID:                    whereHelperint{field: "[dbo].[import_products_marks].[id]"},
	IDImportProducts:      whereHelpernull_Int{field: "[dbo].[import_products_marks].[id_import_products]"},
	IDImportProductsBoxes: whereHelpernull_Int{field: "[dbo].[import_products_marks].[id_import_products_boxes]"},
	Mark:                  whereHelpernull_String{field: "[dbo].[import_products_marks].[mark]"},
}
...
var (
	...
	importProductsMarkColumnsWithoutDefault = []string{"id_import_products", "id_import_products_boxes", "mark"}
	importProductsMarkColumnsWithDefault    = []string{"id"}
...
func ImportProductsMarks(mods ...qm.QueryMod) importProductsMarkQuery {
	mods = append(mods, qm.From("[dbo].[import_products_marks]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[import_products_marks].*"})
	}
...
func FindImportProductsMarkG(ctx context.Context, iD int, selectCols ...string) (*ImportProductsMark, error) {
...
func FindImportProductsMark(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*ImportProductsMark, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[import_products_marks] where [id]=$1", sel,
	)
...
func (o *ImportProductsMark) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[import_products_marks] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[import_products_marks] %sDEFAULT VALUES%s"
		}
...
func (o *ImportProductsMark) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[import_products_marks] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, importProductsMarkPrimaryKeyColumns),
		)
...
func (o ImportProductsMarkSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[import_products_marks] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, importProductsMarkPrimaryKeyColumns, len(o)))
...
func (o *ImportProductsMark) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *ImportProductsMark) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, importProductsMarkGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(importProductsMarkPrimaryKeyColumns, v) && strmangle.ContainsAny(importProductsMarkColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("a3boil: unable to upsert import_products_marks, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(importProductsMarkAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("a3boil: unable to upsert import_products_marks, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[import_products_marks]", importProductsMarkPrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(importProductsMarkPrimaryKeyColumns))
		copy(whitelist, importProductsMarkPrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(importProductsMarkType, importProductsMarkMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *ImportProductsMark) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[import_products_marks] WHERE [id]=$1"
...
func (o ImportProductsMarkSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[import_products_marks] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, importProductsMarkPrimaryKeyColumns, len(o))
...
func (o *ImportProductsMarkSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[import_products_marks].* FROM [dbo].[import_products_marks] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, importProductsMarkPrimaryKeyColumns, len(*o))
...
func ImportProductsMarkExistsG(ctx context.Context, iD int) (bool, error) {
...
func ImportProductsMarkExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[import_products_marks] where [id]=$1) then 1 else 0 end"
