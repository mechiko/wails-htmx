...
var WriteOffRequestWhere = struct {
	...
	ID:             whereHelperint{field: "[dbo].[write_off_requests].[id]"},
	IDWriteOffActs: whereHelpernull_Int{field: "[dbo].[write_off_requests].[id_write_off_acts]"},
	ClientRegID:    whereHelpernull_String{field: "[dbo].[write_off_requests].[client_reg_id]"},
	RequestNumber:  whereHelpernull_String{field: "[dbo].[write_off_requests].[request_number]"},
	RequestDate:    whereHelpernull_String{field: "[dbo].[write_off_requests].[request_date]"},
	RequestRegID:   whereHelpernull_String{field: "[dbo].[write_off_requests].[request_reg_id]"},
	Status:         whereHelpernull_String{field: "[dbo].[write_off_requests].[status]"},
	ReplyID:        whereHelpernull_String{field: "[dbo].[write_off_requests].[reply_id]"},
}
...
var (
	...
	writeOffRequestColumnsWithoutDefault = []string{"id_write_off_acts", "client_reg_id", "request_number", "request_date", "request_reg_id", "status", "reply_id"}
	writeOffRequestColumnsWithDefault    = []string{"id"}
...
func WriteOffRequests(mods ...qm.QueryMod) writeOffRequestQuery {
	mods = append(mods, qm.From("[dbo].[write_off_requests]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[write_off_requests].*"})
	}
...
func FindWriteOffRequestG(ctx context.Context, iD int, selectCols ...string) (*WriteOffRequest, error) {
...
func FindWriteOffRequest(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*WriteOffRequest, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[write_off_requests] where [id]=$1", sel,
	)
...
func (o *WriteOffRequest) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[write_off_requests] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[write_off_requests] %sDEFAULT VALUES%s"
		}
...
func (o *WriteOffRequest) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[write_off_requests] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, writeOffRequestPrimaryKeyColumns),
		)
...
func (o WriteOffRequestSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[write_off_requests] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, writeOffRequestPrimaryKeyColumns, len(o)))
...
func (o *WriteOffRequest) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *WriteOffRequest) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, writeOffRequestGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(writeOffRequestPrimaryKeyColumns, v) && strmangle.ContainsAny(writeOffRequestColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("a3boil: unable to upsert write_off_requests, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(writeOffRequestAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("a3boil: unable to upsert write_off_requests, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[write_off_requests]", writeOffRequestPrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(writeOffRequestPrimaryKeyColumns))
		copy(whitelist, writeOffRequestPrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(writeOffRequestType, writeOffRequestMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *WriteOffRequest) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[write_off_requests] WHERE [id]=$1"
...
func (o WriteOffRequestSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[write_off_requests] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, writeOffRequestPrimaryKeyColumns, len(o))
...
func (o *WriteOffRequestSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[write_off_requests].* FROM [dbo].[write_off_requests] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, writeOffRequestPrimaryKeyColumns, len(*o))
...
func WriteOffRequestExistsG(ctx context.Context, iD int) (bool, error) {
	return WriteOffRequestExists(ctx, boil.GetContextDB(), iD)
}

// WriteOffRequestExists checks if the WriteOffRequest row exists.
func WriteOffRequestExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[write_off_requests] where [id]=$1) then 1 else 0 end"
