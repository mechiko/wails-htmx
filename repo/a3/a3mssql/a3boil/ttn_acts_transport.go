...
var TTNActsTransportWhere = struct {
	...
	ID:        whereHelperint{field: "[dbo].[ttn_acts_transport].[id]"},
	IDTTNActs: whereHelpernull_Int{field: "[dbo].[ttn_acts_transport].[id_ttn_acts]"},
	Ownership: whereHelpernull_String{field: "[dbo].[ttn_acts_transport].[ownership]"},
}
...
var (
	...
	ttnActsTransportColumnsWithoutDefault = []string{"id_ttn_acts", "ownership"}
	ttnActsTransportColumnsWithDefault    = []string{"id"}
...
func TTNActsTransports(mods ...qm.QueryMod) ttnActsTransportQuery {
	mods = append(mods, qm.From("[dbo].[ttn_acts_transport]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[ttn_acts_transport].*"})
	}
...
func FindTTNActsTransportG(ctx context.Context, iD int, selectCols ...string) (*TTNActsTransport, error) {
...
func FindTTNActsTransport(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*TTNActsTransport, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[ttn_acts_transport] where [id]=$1", sel,
	)
...
func (o *TTNActsTransport) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[ttn_acts_transport] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[ttn_acts_transport] %sDEFAULT VALUES%s"
		}
...
func (o *TTNActsTransport) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[ttn_acts_transport] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, ttnActsTransportPrimaryKeyColumns),
		)
...
func (o TTNActsTransportSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[ttn_acts_transport] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, ttnActsTransportPrimaryKeyColumns, len(o)))
...
func (o *TTNActsTransport) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *TTNActsTransport) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, ttnActsTransportGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(ttnActsTransportPrimaryKeyColumns, v) && strmangle.ContainsAny(ttnActsTransportColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("a3boil: unable to upsert ttn_acts_transport, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(ttnActsTransportAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("a3boil: unable to upsert ttn_acts_transport, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[ttn_acts_transport]", ttnActsTransportPrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(ttnActsTransportPrimaryKeyColumns))
		copy(whitelist, ttnActsTransportPrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(ttnActsTransportType, ttnActsTransportMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *TTNActsTransport) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[ttn_acts_transport] WHERE [id]=$1"
...
func (o TTNActsTransportSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[ttn_acts_transport] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, ttnActsTransportPrimaryKeyColumns, len(o))
...
func (o *TTNActsTransportSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[ttn_acts_transport].* FROM [dbo].[ttn_acts_transport] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, ttnActsTransportPrimaryKeyColumns, len(*o))
...
func TTNActsTransportExistsG(ctx context.Context, iD int) (bool, error) {
	return TTNActsTransportExists(ctx, boil.GetContextDB(), iD)
}

// TTNActsTransportExists checks if the TTNActsTransport row exists.
func TTNActsTransportExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[ttn_acts_transport] where [id]=$1) then 1 else 0 end"
