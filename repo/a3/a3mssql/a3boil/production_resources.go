...
var ProductionResourceWhere = struct {
	...
	ID:                    whereHelperint{field: "[dbo].[production_resources].[id]"},
	IDProductionReports:   whereHelpernull_Int{field: "[dbo].[production_resources].[id_production_reports]"},
	IDProductionProducts:  whereHelpernull_Int{field: "[dbo].[production_resources].[id_production_products]"},
	ResourceFullName:      whereHelpernull_String{field: "[dbo].[production_resources].[resource_full_name]"},
	ResourceCapacity:      whereHelpernull_String{field: "[dbo].[production_resources].[resource_capacity]"},
	ResourceAlcVolume:     whereHelpernull_String{field: "[dbo].[production_resources].[resource_alc_volume]"},
	...
	ProducerType:          whereHelpernull_String{field: "[dbo].[production_resources].[producer_type]"},
	ProducerClientRegID:   whereHelpernull_String{field: "[dbo].[production_resources].[producer_client_reg_id]"},
	ProducerInn:           whereHelpernull_String{field: "[dbo].[production_resources].[producer_inn]"},
	ProducerKPP:           whereHelpernull_String{field: "[dbo].[production_resources].[producer_kpp]"},
	ProducerFullName:      whereHelpernull_String{field: "[dbo].[production_resources].[producer_full_name]"},
	ProducerShortName:     whereHelpernull_String{field: "[dbo].[production_resources].[producer_short_name]"},
	ProducerCountryCode:   whereHelpernull_String{field: "[dbo].[production_resources].[producer_country_code]"},
	ProducerRegionCode:    whereHelpernull_String{field: "[dbo].[production_resources].[producer_region_code]"},
	ProducerDescription:   whereHelpernull_String{field: "[dbo].[production_resources].[producer_description]"},
}
...
func ProductionResources(mods ...qm.QueryMod) productionResourceQuery {
	mods = append(mods, qm.From("[dbo].[production_resources]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[production_resources].*"})
	}
...
func FindProductionResourceG(ctx context.Context, iD int, selectCols ...string) (*ProductionResource, error) {
...
func FindProductionResource(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*ProductionResource, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[production_resources] where [id]=$1", sel,
	)
...
func (o *ProductionResource) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[production_resources] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[production_resources] %sDEFAULT VALUES%s"
		}
...
func (o *ProductionResource) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[production_resources] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, productionResourcePrimaryKeyColumns),
		)
...
func (o ProductionResourceSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[production_resources] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, productionResourcePrimaryKeyColumns, len(o)))
...
func (o *ProductionResource) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *ProductionResource) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, productionResourceGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(productionResourcePrimaryKeyColumns, v) && strmangle.ContainsAny(productionResourceColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("a3boil: unable to upsert production_resources, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(productionResourceAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("a3boil: unable to upsert production_resources, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[production_resources]", productionResourcePrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(productionResourcePrimaryKeyColumns))
		copy(whitelist, productionResourcePrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(productionResourceType, productionResourceMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *ProductionResource) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[production_resources] WHERE [id]=$1"
...
func (o ProductionResourceSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[production_resources] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, productionResourcePrimaryKeyColumns, len(o))
...
func (o *ProductionResourceSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[production_resources].* FROM [dbo].[production_resources] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, productionResourcePrimaryKeyColumns, len(*o))
...
func ProductionResourceExistsG(ctx context.Context, iD int) (bool, error) {
...
func ProductionResourceExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[production_resources] where [id]=$1) then 1 else 0 end"
