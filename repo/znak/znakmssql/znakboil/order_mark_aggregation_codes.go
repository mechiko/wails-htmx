...
var OrderMarkAggregationCodeWhere = struct {
	...
	ID:                     whereHelperint{field: "[dbo].[order_mark_aggregation_codes].[id]"},
	IDOrderMarkAggregation: whereHelpernull_Int{field: "[dbo].[order_mark_aggregation_codes].[id_order_mark_aggregation]"},
	SerialNumber:           whereHelpernull_String{field: "[dbo].[order_mark_aggregation_codes].[serial_number]"},
	Code:                   whereHelpernull_String{field: "[dbo].[order_mark_aggregation_codes].[code]"},
	Status:                 whereHelpernull_String{field: "[dbo].[order_mark_aggregation_codes].[status]"},
}
...
var (
	...
	orderMarkAggregationCodeColumnsWithoutDefault = []string{"id_order_mark_aggregation", "serial_number", "code", "status"}
	orderMarkAggregationCodeColumnsWithDefault    = []string{"id"}
...
func OrderMarkAggregationCodes(mods ...qm.QueryMod) orderMarkAggregationCodeQuery {
	mods = append(mods, qm.From("[dbo].[order_mark_aggregation_codes]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[order_mark_aggregation_codes].*"})
	}
...
func FindOrderMarkAggregationCodeG(ctx context.Context, iD int, selectCols ...string) (*OrderMarkAggregationCode, error) {
...
func FindOrderMarkAggregationCode(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*OrderMarkAggregationCode, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[order_mark_aggregation_codes] where [id]=$1", sel,
	)
...
func (o *OrderMarkAggregationCode) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[order_mark_aggregation_codes] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[order_mark_aggregation_codes] %sDEFAULT VALUES%s"
		}
...
func (o *OrderMarkAggregationCode) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[order_mark_aggregation_codes] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, orderMarkAggregationCodePrimaryKeyColumns),
		)
...
func (o OrderMarkAggregationCodeSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[order_mark_aggregation_codes] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, orderMarkAggregationCodePrimaryKeyColumns, len(o)))
...
func (o *OrderMarkAggregationCode) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *OrderMarkAggregationCode) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, orderMarkAggregationCodeGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(orderMarkAggregationCodePrimaryKeyColumns, v) && strmangle.ContainsAny(orderMarkAggregationCodeColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("znakboil: unable to upsert order_mark_aggregation_codes, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(orderMarkAggregationCodeAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("znakboil: unable to upsert order_mark_aggregation_codes, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[order_mark_aggregation_codes]", orderMarkAggregationCodePrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(orderMarkAggregationCodePrimaryKeyColumns))
		copy(whitelist, orderMarkAggregationCodePrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(orderMarkAggregationCodeType, orderMarkAggregationCodeMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *OrderMarkAggregationCode) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[order_mark_aggregation_codes] WHERE [id]=$1"
...
func (o OrderMarkAggregationCodeSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[order_mark_aggregation_codes] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, orderMarkAggregationCodePrimaryKeyColumns, len(o))
...
func (o *OrderMarkAggregationCodeSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[order_mark_aggregation_codes].* FROM [dbo].[order_mark_aggregation_codes] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, orderMarkAggregationCodePrimaryKeyColumns, len(*o))
...
func OrderMarkAggregationCodeExistsG(ctx context.Context, iD int) (bool, error) {
...
func OrderMarkAggregationCodeExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[order_mark_aggregation_codes] where [id]=$1) then 1 else 0 end"
