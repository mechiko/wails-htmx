...
var OrderMarkCodesSerialNumberWhere = struct {
	...
	ID:               whereHelperint{field: "[dbo].[order_mark_codes_serial_numbers].[id]"},
	IDOrderMarkCodes: whereHelpernull_Int{field: "[dbo].[order_mark_codes_serial_numbers].[id_order_mark_codes]"},
	Gtin:             whereHelpernull_String{field: "[dbo].[order_mark_codes_serial_numbers].[gtin]"},
	SerialNumber:     whereHelpernull_String{field: "[dbo].[order_mark_codes_serial_numbers].[serial_number]"},
	Code:             whereHelpernull_String{field: "[dbo].[order_mark_codes_serial_numbers].[code]"},
	BlockID:          whereHelpernull_String{field: "[dbo].[order_mark_codes_serial_numbers].[block_id]"},
	Status:           whereHelpernull_String{field: "[dbo].[order_mark_codes_serial_numbers].[status]"},
}
...
var (
	...
	orderMarkCodesSerialNumberColumnsWithoutDefault = []string{"id_order_mark_codes", "gtin", "serial_number", "code", "block_id", "status"}
	orderMarkCodesSerialNumberColumnsWithDefault    = []string{"id"}
...
func OrderMarkCodesSerialNumbers(mods ...qm.QueryMod) orderMarkCodesSerialNumberQuery {
	mods = append(mods, qm.From("[dbo].[order_mark_codes_serial_numbers]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[order_mark_codes_serial_numbers].*"})
	}
...
func FindOrderMarkCodesSerialNumberG(ctx context.Context, iD int, selectCols ...string) (*OrderMarkCodesSerialNumber, error) {
...
func FindOrderMarkCodesSerialNumber(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*OrderMarkCodesSerialNumber, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[order_mark_codes_serial_numbers] where [id]=$1", sel,
	)
...
func (o *OrderMarkCodesSerialNumber) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[order_mark_codes_serial_numbers] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[order_mark_codes_serial_numbers] %sDEFAULT VALUES%s"
		}
...
func (o *OrderMarkCodesSerialNumber) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[order_mark_codes_serial_numbers] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, orderMarkCodesSerialNumberPrimaryKeyColumns),
		)
...
func (o OrderMarkCodesSerialNumberSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[order_mark_codes_serial_numbers] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, orderMarkCodesSerialNumberPrimaryKeyColumns, len(o)))
...
func (o *OrderMarkCodesSerialNumber) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *OrderMarkCodesSerialNumber) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, orderMarkCodesSerialNumberGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(orderMarkCodesSerialNumberPrimaryKeyColumns, v) && strmangle.ContainsAny(orderMarkCodesSerialNumberColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("znakboil: unable to upsert order_mark_codes_serial_numbers, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(orderMarkCodesSerialNumberAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("znakboil: unable to upsert order_mark_codes_serial_numbers, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[order_mark_codes_serial_numbers]", orderMarkCodesSerialNumberPrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(orderMarkCodesSerialNumberPrimaryKeyColumns))
		copy(whitelist, orderMarkCodesSerialNumberPrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(orderMarkCodesSerialNumberType, orderMarkCodesSerialNumberMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *OrderMarkCodesSerialNumber) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[order_mark_codes_serial_numbers] WHERE [id]=$1"
...
func (o OrderMarkCodesSerialNumberSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[order_mark_codes_serial_numbers] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, orderMarkCodesSerialNumberPrimaryKeyColumns, len(o))
...
func (o *OrderMarkCodesSerialNumberSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[order_mark_codes_serial_numbers].* FROM [dbo].[order_mark_codes_serial_numbers] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, orderMarkCodesSerialNumberPrimaryKeyColumns, len(*o))
...
func OrderMarkCodesSerialNumberExistsG(ctx context.Context, iD int) (bool, error) {
...
func OrderMarkCodesSerialNumberExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[order_mark_codes_serial_numbers] where [id]=$1) then 1 else 0 end"
