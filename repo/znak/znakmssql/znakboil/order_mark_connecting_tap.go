...
var OrderMarkConnectingTapWhere = struct {
	...
	ID:         whereHelperint{field: "[dbo].[order_mark_connecting_tap].[id]"},
	CreateDate: whereHelpernull_String{field: "[dbo].[order_mark_connecting_tap].[create_date]"},
	Quantity:   whereHelpernull_String{field: "[dbo].[order_mark_connecting_tap].[quantity]"},
	Inn:        whereHelpernull_String{field: "[dbo].[order_mark_connecting_tap].[inn]"},
	KPP:        whereHelpernull_String{field: "[dbo].[order_mark_connecting_tap].[kpp]"},
	Version:    whereHelpernull_String{field: "[dbo].[order_mark_connecting_tap].[version]"},
	State:      whereHelpernull_String{field: "[dbo].[order_mark_connecting_tap].[state]"},
	Status:     whereHelpernull_String{field: "[dbo].[order_mark_connecting_tap].[status]"},
	OrderID:    whereHelpernull_String{field: "[dbo].[order_mark_connecting_tap].[order_id]"},
	Archive:    whereHelpernull_Int{field: "[dbo].[order_mark_connecting_tap].[archive]"},
	JSON:       whereHelpernull_String{field: "[dbo].[order_mark_connecting_tap].[json]"},
}
...
var (
	...
	orderMarkConnectingTapColumnsWithoutDefault = []string{"create_date", "quantity", "inn", "kpp", "version", "state", "status", "order_id", "archive", "json"}
	orderMarkConnectingTapColumnsWithDefault    = []string{"id"}
...
func OrderMarkConnectingTaps(mods ...qm.QueryMod) orderMarkConnectingTapQuery {
	mods = append(mods, qm.From("[dbo].[order_mark_connecting_tap]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[order_mark_connecting_tap].*"})
	}
...
func FindOrderMarkConnectingTapG(ctx context.Context, iD int, selectCols ...string) (*OrderMarkConnectingTap, error) {
...
func FindOrderMarkConnectingTap(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*OrderMarkConnectingTap, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[order_mark_connecting_tap] where [id]=$1", sel,
	)
...
func (o *OrderMarkConnectingTap) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[order_mark_connecting_tap] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[order_mark_connecting_tap] %sDEFAULT VALUES%s"
		}
...
func (o *OrderMarkConnectingTap) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[order_mark_connecting_tap] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, orderMarkConnectingTapPrimaryKeyColumns),
		)
...
func (o OrderMarkConnectingTapSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[order_mark_connecting_tap] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, orderMarkConnectingTapPrimaryKeyColumns, len(o)))
...
func (o *OrderMarkConnectingTap) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *OrderMarkConnectingTap) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, orderMarkConnectingTapGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(orderMarkConnectingTapPrimaryKeyColumns, v) && strmangle.ContainsAny(orderMarkConnectingTapColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("znakboil: unable to upsert order_mark_connecting_tap, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(orderMarkConnectingTapAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("znakboil: unable to upsert order_mark_connecting_tap, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[order_mark_connecting_tap]", orderMarkConnectingTapPrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(orderMarkConnectingTapPrimaryKeyColumns))
		copy(whitelist, orderMarkConnectingTapPrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(orderMarkConnectingTapType, orderMarkConnectingTapMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *OrderMarkConnectingTap) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[order_mark_connecting_tap] WHERE [id]=$1"
...
func (o OrderMarkConnectingTapSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[order_mark_connecting_tap] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, orderMarkConnectingTapPrimaryKeyColumns, len(o))
...
func (o *OrderMarkConnectingTapSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[order_mark_connecting_tap].* FROM [dbo].[order_mark_connecting_tap] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, orderMarkConnectingTapPrimaryKeyColumns, len(*o))
...
func OrderMarkConnectingTapExistsG(ctx context.Context, iD int) (bool, error) {
...
func OrderMarkConnectingTapExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[order_mark_connecting_tap] where [id]=$1) then 1 else 0 end"
