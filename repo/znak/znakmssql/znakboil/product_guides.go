...
var ProductGuideWhere = struct {
	...
	ID:                 whereHelperint{field: "[dbo].[product_guides].[id]"},
	ProductName:        whereHelpernull_String{field: "[dbo].[product_guides].[product_name]"},
	ProductGtin:        whereHelpernull_String{field: "[dbo].[product_guides].[product_gtin]"},
	ProductTemplate:    whereHelpernull_String{field: "[dbo].[product_guides].[product_template]"},
	ProductCapacity:    whereHelpernull_String{field: "[dbo].[product_guides].[product_capacity]"},
	ProductShelfLife:   whereHelpernull_String{field: "[dbo].[product_guides].[product_shelf_life]"},
	ProductAlcCode:     whereHelpernull_String{field: "[dbo].[product_guides].[product_alc_code]"},
	TNVed:              whereHelpernull_String{field: "[dbo].[product_guides].[tn_ved]"},
	DeclarationNumber:  whereHelpernull_String{field: "[dbo].[product_guides].[declaration_number]"},
	DeclarationDate:    whereHelpernull_String{field: "[dbo].[product_guides].[declaration_date]"},
	CertificateNumber:  whereHelpernull_String{field: "[dbo].[product_guides].[certificate_number]"},
	CertificateDate:    whereHelpernull_String{field: "[dbo].[product_guides].[certificate_date]"},
	StateNumber:        whereHelpernull_String{field: "[dbo].[product_guides].[state_number]"},
	StateDate:          whereHelpernull_String{field: "[dbo].[product_guides].[state_date]"},
	WaterLicenseNumber: whereHelpernull_String{field: "[dbo].[product_guides].[water_license_number]"},
	WaterLicenseDate:   whereHelpernull_String{field: "[dbo].[product_guides].[water_license_date]"},
	ProductRegID:       whereHelpernull_String{field: "[dbo].[product_guides].[product_reg_id]"},
}
...
var (
	productGuideAllColumns            = []string{"id", "product_name", "product_gtin", "product_template", "product_capacity", "product_shelf_life", "product_alc_code", "tn_ved", "declaration_number", "declaration_date", "certificate_number", "certificate_date", "state_number", "state_date", "water_license_number", "water_license_date", "product_reg_id"}
	productGuideColumnsWithoutDefault = []string{"product_name", "product_gtin", "product_template", "product_capacity", "product_shelf_life", "product_alc_code", "tn_ved", "declaration_number", "declaration_date", "certificate_number", "certificate_date", "state_number", "state_date", "water_license_number", "water_license_date", "product_reg_id"}
	productGuideColumnsWithDefault    = []string{"id"}
...
func ProductGuides(mods ...qm.QueryMod) productGuideQuery {
	mods = append(mods, qm.From("[dbo].[product_guides]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[product_guides].*"})
	}
...
func FindProductGuideG(ctx context.Context, iD int, selectCols ...string) (*ProductGuide, error) {
...
func FindProductGuide(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*ProductGuide, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[product_guides] where [id]=$1", sel,
	)
...
func (o *ProductGuide) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[product_guides] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[product_guides] %sDEFAULT VALUES%s"
		}
...
func (o *ProductGuide) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[product_guides] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, productGuidePrimaryKeyColumns),
		)
...
func (o ProductGuideSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[product_guides] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, productGuidePrimaryKeyColumns, len(o)))
...
func (o *ProductGuide) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *ProductGuide) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, productGuideGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(productGuidePrimaryKeyColumns, v) && strmangle.ContainsAny(productGuideColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("znakboil: unable to upsert product_guides, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(productGuideAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("znakboil: unable to upsert product_guides, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[product_guides]", productGuidePrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(productGuidePrimaryKeyColumns))
		copy(whitelist, productGuidePrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(productGuideType, productGuideMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *ProductGuide) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[product_guides] WHERE [id]=$1"
...
func (o ProductGuideSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[product_guides] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, productGuidePrimaryKeyColumns, len(o))
...
func (o *ProductGuideSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[product_guides].* FROM [dbo].[product_guides] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, productGuidePrimaryKeyColumns, len(*o))
...
func ProductGuideExistsG(ctx context.Context, iD int) (bool, error) {
	return ProductGuideExists(ctx, boil.GetContextDB(), iD)
}

// ProductGuideExists checks if the ProductGuide row exists.
func ProductGuideExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[product_guides] where [id]=$1) then 1 else 0 end"
