...
var ProductCardsGtinWhere = struct {
	...
	ID:             whereHelperint{field: "[dbo].[product_cards_gtins].[id]"},
	IDProductCards: whereHelpernull_Int{field: "[dbo].[product_cards_gtins].[id_product_cards]"},
	Gtin:           whereHelpernull_String{field: "[dbo].[product_cards_gtins].[gtin]"},
	Quantity:       whereHelpernull_Int{field: "[dbo].[product_cards_gtins].[quantity]"},
}
...
var (
	...
	productCardsGtinColumnsWithoutDefault = []string{"id_product_cards", "gtin", "quantity"}
	productCardsGtinColumnsWithDefault    = []string{"id"}
...
func ProductCardsGtins(mods ...qm.QueryMod) productCardsGtinQuery {
	mods = append(mods, qm.From("[dbo].[product_cards_gtins]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[product_cards_gtins].*"})
	}
...
func FindProductCardsGtinG(ctx context.Context, iD int, selectCols ...string) (*ProductCardsGtin, error) {
...
func FindProductCardsGtin(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*ProductCardsGtin, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[product_cards_gtins] where [id]=$1", sel,
	)
...
func (o *ProductCardsGtin) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[product_cards_gtins] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[product_cards_gtins] %sDEFAULT VALUES%s"
		}
...
func (o *ProductCardsGtin) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[product_cards_gtins] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, productCardsGtinPrimaryKeyColumns),
		)
...
func (o ProductCardsGtinSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[product_cards_gtins] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, productCardsGtinPrimaryKeyColumns, len(o)))
...
func (o *ProductCardsGtin) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *ProductCardsGtin) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, productCardsGtinGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(productCardsGtinPrimaryKeyColumns, v) && strmangle.ContainsAny(productCardsGtinColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("znakboil: unable to upsert product_cards_gtins, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(productCardsGtinAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("znakboil: unable to upsert product_cards_gtins, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[product_cards_gtins]", productCardsGtinPrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(productCardsGtinPrimaryKeyColumns))
		copy(whitelist, productCardsGtinPrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(productCardsGtinType, productCardsGtinMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *ProductCardsGtin) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[product_cards_gtins] WHERE [id]=$1"
...
func (o ProductCardsGtinSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[product_cards_gtins] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, productCardsGtinPrimaryKeyColumns, len(o))
...
func (o *ProductCardsGtinSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[product_cards_gtins].* FROM [dbo].[product_cards_gtins] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, productCardsGtinPrimaryKeyColumns, len(*o))
...
func ProductCardsGtinExistsG(ctx context.Context, iD int) (bool, error) {
	return ProductCardsGtinExists(ctx, boil.GetContextDB(), iD)
}

// ProductCardsGtinExists checks if the ProductCardsGtin row exists.
func ProductCardsGtinExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[product_cards_gtins] where [id]=$1) then 1 else 0 end"
