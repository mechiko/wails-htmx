...
var OrderMarkUtilisationWhere = struct {
	...
	ID:               whereHelperint{field: "[dbo].[order_mark_utilisation].[id]"},
	IDOrderMarkCodes: whereHelpernull_Int{field: "[dbo].[order_mark_utilisation].[id_order_mark_codes]"},
	CreateDate:       whereHelpernull_String{field: "[dbo].[order_mark_utilisation].[create_date]"},
	ProductionDate:   whereHelpernull_String{field: "[dbo].[order_mark_utilisation].[production_date]"},
	ExpirationDate:   whereHelpernull_String{field: "[dbo].[order_mark_utilisation].[expiration_date]"},
	UsageType:        whereHelpernull_String{field: "[dbo].[order_mark_utilisation].[usage_type]"},
	Inn:              whereHelpernull_String{field: "[dbo].[order_mark_utilisation].[inn]"},
	KPP:              whereHelpernull_String{field: "[dbo].[order_mark_utilisation].[kpp]"},
	Quantity:         whereHelpernull_String{field: "[dbo].[order_mark_utilisation].[quantity]"},
	Version:          whereHelpernull_String{field: "[dbo].[order_mark_utilisation].[version]"},
	State:            whereHelpernull_String{field: "[dbo].[order_mark_utilisation].[state]"},
	Status:           whereHelpernull_String{field: "[dbo].[order_mark_utilisation].[status]"},
	ReportID:         whereHelpernull_String{field: "[dbo].[order_mark_utilisation].[report_id]"},
	Archive:          whereHelpernull_Int{field: "[dbo].[order_mark_utilisation].[archive]"},
	JSON:             whereHelpernull_String{field: "[dbo].[order_mark_utilisation].[json]"},
}
...
var (
	orderMarkUtilisationAllColumns            = []string{"id", "id_order_mark_codes", "create_date", "production_date", "expiration_date", "usage_type", "inn", "kpp", "quantity", "version", "state", "status", "report_id", "archive", "json"}
	orderMarkUtilisationColumnsWithoutDefault = []string{"id_order_mark_codes", "create_date", "production_date", "expiration_date", "usage_type", "inn", "kpp", "quantity", "version", "state", "status", "report_id", "archive", "json"}
	orderMarkUtilisationColumnsWithDefault    = []string{"id"}
...
func OrderMarkUtilisations(mods ...qm.QueryMod) orderMarkUtilisationQuery {
	mods = append(mods, qm.From("[dbo].[order_mark_utilisation]"))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"[dbo].[order_mark_utilisation].*"})
	}
...
func FindOrderMarkUtilisationG(ctx context.Context, iD int, selectCols ...string) (*OrderMarkUtilisation, error) {
...
func FindOrderMarkUtilisation(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*OrderMarkUtilisation, error) {
	...
	query := fmt.Sprintf(
		"select %s from [dbo].[order_mark_utilisation] where [id]=$1", sel,
	)
...
func (o *OrderMarkUtilisation) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO [dbo].[order_mark_utilisation] ([%s]) %%sVALUES (%s)%%s", strings.Join(wl, "],["), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO [dbo].[order_mark_utilisation] %sDEFAULT VALUES%s"
		}
...
func (o *OrderMarkUtilisation) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE [dbo].[order_mark_utilisation] SET %s WHERE %s",
			strmangle.SetParamNames("[", "]", 1, wl),
			strmangle.WhereClause("[", "]", len(wl)+1, orderMarkUtilisationPrimaryKeyColumns),
		)
...
func (o OrderMarkUtilisationSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE [dbo].[order_mark_utilisation] SET %s WHERE %s",
		strmangle.SetParamNames("[", "]", 1, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, orderMarkUtilisationPrimaryKeyColumns, len(o)))
...
func (o *OrderMarkUtilisation) UpsertG(ctx context.Context, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
func (o *OrderMarkUtilisation) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, orderMarkUtilisationGeneratedColumns)

		for i, v := range insert {
			if strmangle.ContainsAny(orderMarkUtilisationPrimaryKeyColumns, v) && strmangle.ContainsAny(orderMarkUtilisationColumnsWithDefault, v) {
				insert = append(insert[:i], insert[i+1:]...)
			}
		}
		if len(insert) == 0 {
			return errors.New("znakboil: unable to upsert order_mark_utilisation, could not build insert column list")
		}
		...
		ret := strmangle.SetComplement(orderMarkUtilisationAllColumns, strmangle.SetIntersect(insert, update))

		if !updateColumns.IsNone() && len(update) == 0 {
			return errors.New("znakboil: unable to upsert order_mark_utilisation, could not build update column list")
		}

		cache.query = buildUpsertQueryMSSQL(dialect, "[dbo].[order_mark_utilisation]", orderMarkUtilisationPrimaryKeyColumns, update, insert, ret)

		whitelist := make([]string, len(orderMarkUtilisationPrimaryKeyColumns))
		copy(whitelist, orderMarkUtilisationPrimaryKeyColumns)
		whitelist = append(whitelist, update...)
		whitelist = append(whitelist, insert...)

		cache.valueMapping, err = queries.BindMapping(orderMarkUtilisationType, orderMarkUtilisationMapping, whitelist)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *OrderMarkUtilisation) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[order_mark_utilisation] WHERE [id]=$1"
...
func (o OrderMarkUtilisationSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM [dbo].[order_mark_utilisation] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, orderMarkUtilisationPrimaryKeyColumns, len(o))
...
func (o *OrderMarkUtilisationSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT [dbo].[order_mark_utilisation].* FROM [dbo].[order_mark_utilisation] WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, orderMarkUtilisationPrimaryKeyColumns, len(*o))
...
func OrderMarkUtilisationExistsG(ctx context.Context, iD int) (bool, error) {
...
func OrderMarkUtilisationExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select case when exists(select top(1) 1 from [dbo].[order_mark_utilisation] where [id]=$1) then 1 else 0 end"
