...
type ProductCard struct {
	...
	GoodID     null.Int64  `boil:"good_id" json:"good_id,omitempty" toml:"good_id" yaml:"good_id,omitempty"`
	...
	GoodRating null.Int64  `boil:"good_rating" json:"good_rating,omitempty" toml:"good_rating" yaml:"good_rating,omitempty"`
...
var ProductCardWhere = struct {
	...
	GoodRating whereHelpernull_Int64
}{
	ID:         whereHelperint64{field: "\"product_cards\".\"id\""},
	Gtin:       whereHelpernull_String{field: "\"product_cards\".\"gtin\""},
	GoodID:     whereHelpernull_Int64{field: "\"product_cards\".\"good_id\""},
	GoodName:   whereHelpernull_String{field: "\"product_cards\".\"good_name\""},
	IsKit:      whereHelpernull_String{field: "\"product_cards\".\"is_kit\""},
	IsSet:      whereHelpernull_String{field: "\"product_cards\".\"is_set\""},
	GoodURL:    whereHelpernull_String{field: "\"product_cards\".\"good_url\""},
	GoodImg:    whereHelpernull_String{field: "\"product_cards\".\"good_img\""},
	GoodStatus: whereHelpernull_String{field: "\"product_cards\".\"good_status\""},
	CreateDate: whereHelpernull_String{field: "\"product_cards\".\"create_date\""},
	UpdateDate: whereHelpernull_String{field: "\"product_cards\".\"update_date\""},
	BrandID:    whereHelpernull_Int64{field: "\"product_cards\".\"brand_id\""},
	BrandName:  whereHelpernull_String{field: "\"product_cards\".\"brand_name\""},
	GoodRating: whereHelpernull_Int64{field: "\"product_cards\".\"good_rating\""},
}
...
var (
	...
	productCardColumnsWithoutDefault = []string{}
	productCardColumnsWithDefault    = []string{"id", "gtin", "good_id", "good_name", "is_kit", "is_set", "good_url", "good_img", "good_status", "create_date", "update_date", "brand_id", "brand_name", "good_rating"}
...
func ProductCards(mods ...qm.QueryMod) productCardQuery {
	mods = append(mods, qm.From("\"product_cards\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"product_cards\".*"})
	}
...
func FindProductCardG(ctx context.Context, iD int64, selectCols ...string) (*ProductCard, error) {
...
func FindProductCard(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*ProductCard, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"product_cards\" where \"id\"=?", sel,
	)
...
func (o *ProductCard) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"product_cards\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"product_cards\" %sDEFAULT VALUES%s"
		}
...
func (o *ProductCard) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"product_cards\" SET %s WHERE %s",
			strmangle.SetParamNames("\"", "\"", 0, wl),
			strmangle.WhereClause("\"", "\"", 0, productCardPrimaryKeyColumns),
		)
...
func (o ProductCardSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"product_cards\" SET %s WHERE %s",
		strmangle.SetParamNames("\"", "\"", 0, colNames),
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, productCardPrimaryKeyColumns, len(o)))
...
func (o *ProductCard) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *ProductCard) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		if len(conflict) == 0 {
			conflict = make([]string, len(productCardPrimaryKeyColumns))
			copy(conflict, productCardPrimaryKeyColumns)
		}
		cache.query = buildUpsertQuerySQLite(dialect, "\"product_cards\"", updateOnConflict, ret, update, conflict, insert)

		cache.valueMapping, err = queries.BindMapping(productCardType, productCardMapping, insert)
		if err != nil {
			return err
		}
		if len(ret) != 0 {
...
func (o *ProductCard) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"product_cards\" WHERE \"id\"=?"
...
func (o ProductCardSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"product_cards\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, productCardPrimaryKeyColumns, len(o))
...
func (o *ProductCardSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"product_cards\".* FROM \"product_cards\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, productCardPrimaryKeyColumns, len(*o))
...
func ProductCardExistsG(ctx context.Context, iD int64) (bool, error) {
	return ProductCardExists(ctx, boil.GetContextDB(), iD)
}

// ProductCardExists checks if the ProductCard row exists.
func ProductCardExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"product_cards\" where \"id\"=? limit 1)"
